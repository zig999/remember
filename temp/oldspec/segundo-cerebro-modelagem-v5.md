# Sistema de Segundo Cérebro — Modelagem v5 (Grafo + Full-text, bitemporal — especificação fechada)
...
>
> A v5 é a v4 após revisão técnica: inconsistências corrigidas, todas as decisões em aberto
> **fechadas** (consolidadas no Apêndice A — ADRs), e as seções operacionais que faltavam
> (catálogo de ferramentas MCP, catálogos seed, fluxo de ingestão, curadoria, máquinas de
> estado, NFRs e cenários de aceitação) **redigidas por extenso**. Não há "ex.:" decisório
> neste documento: onde havia alternativas, há escolha única registrada.
>
> **Escopo da fase 1:** grafo + full-text **puro**. A LLM atua apenas na ingestão; a consulta é
> 100% determinística. Não há embeddings nem banco vetorial — a evolução para busca semântica é
> aditiva e está descrita na seção 20.
...
5. **Mantém histórico temporal** das mudanças (bitemporal): separa "verdade atual" de
   "histórico de validade" e de "o que o sistema sabia em cada instante".
...
- O **MCP Server** é a fronteira: a LLM só age através das ferramentas dele, organizadas em três
  toolsets: `ingest`, `query`, `curation`.
...
- **Camada de ingestão (escrita):** transforma informação bruta em conhecimento estruturado. A
  LLM participa aqui (seção 9 descreve o fluxo ponta-a-ponta).
- **Camada de consulta (leitura):** responde perguntas via full-text + grafo (seção 7). A LLM,
  quando presente, atua como **orquestradora/redatora**: chama as ferramentas determinísticas do
  toolset `query`, recebe resultados + proveniência e compõe a resposta — ela **não** faz a
...
| Aspecto | Decisão |
|---|---|
| Banco | **PostgreSQL 17** — store único; nenhum outro serviço de busca/armazenamento |
...
| Acesso a dados | Query builder próximo do SQL (**Kysely** ou **Drizzle**) + migrações SQL puras versionadas no repositório |
| Vetores (futuro) | `pgvector`, fase de embeddings (seção 20) — nada instalado na fase 1 |

### 2.3 Operação mono-usuário

O sistema é **mono-usuário por especificação**. Não existe entidade `User`:
...
- O MCP Server opera em transporte local, single-tenant. Não há autenticação de usuários nem
  autorização por papel na fase 1.
- Multiusuário futuro é **aditivo**: acrescenta-se entidade de ator e coluna de autoria às
  trilhas existentes, sem alterar a semântica de nada.
...
- `storage_ref` (text, nulável) — **reservado**; não usado na fase 1
...
- `chunking_version` (text) — `'v1'` nesta fase (estratégia na seção 9.2)
- `text_search` — tsvector **gerado** com a configuração `pt_unaccent_v1` + índice GIN
- `embedding` — **reservado** (nulável), não preenchido nesta fase
...
> Os antigos campos `aliases` e `name_search` **não existem mais** — foram normalizados para
> `NodeAlias` (abaixo).
...
#### AttributeKey *(novo na v5 — governa as chaves de atributo)*
...
o campo `cardinality` da v4 **foi removido** (multiplicidade mora em
`LinkType.allows_multiple_current`; não há regra de precedência a resolver).
...
#### CurationAction *(novo na v5)*
Trilha de auditoria de **toda** ação de curadoria (mono-usuário: ator implícito).

- `id`, `action` (text — nome da ferramenta de curadoria), `target_kind`, `target_id`,
  `payload` (jsonb), `reason` (text), `created_at`

#### ComplianceDeletion *(novo na v5)*
...
Pipeline (estratégia de similaridade **isolada atrás de interface** — ponto de extensão da
seção 20):
...
A v1 usa **só trigram** como fuzzy (sinal único, um threshold a calibrar). Levenshtein fica
documentado como segundo sinal possível se nomes curtos (<6 chars, poucos trigramas) gerarem
falsos negativos na prática — entra pela interface, sem mexer no fluxo.
...
> exatamente os que a fase de embeddings melhora (seção 20).

---

## 5. Modelo Temporal (bitemporal)

O coração do modelo: separar **"verdade atual"**, **"histórico de validade"** e **"o que o
sistema sabia"**. Regra principal: **nada relevante é sobrescrito; o que muda no tempo é
encerrado, e uma nova versão passa a valer.** Vale igualmente para `KnowledgeLink` e
`NodeAttribute`.
...
Exemplo clássico (por que (c) existe): João saiu do projeto em **01/05** (mundo), mas a ata só
chegou em **10/05**. Registra-se `valid_to = 2026-05-01`, `recorded_at = 2026-05-10`. A consulta
(c) com `T = 2026-05-05` ainda mostra João no projeto — era o que o sistema sabia.
...
| `uncertain` | Aceito provisoriamente (confiança 0.40–0.74); aguarda corroboração |
| `disputed` | Em conflito com outra versão no mesmo período |
...
2. Item novo é criado já `disputed`; fila de curadoria (seção 10).
...
é rastreada na v1 (limitação documentada; coluna futura aditiva).

**Corroboração:** a mesma asserção vinda de `RawInformation` **independente** promove
`uncertain → active`; proveniência acumulada; confiança exibida = máx. das fontes.
...
| proposed | confiança < 0.40 e nunca citado | permanece proposed (fila de curadoria) |
...
0.40–0.74; < 0.40 **não cria** — fragmento fica `proposed` na fila):
...
| uncertain | corroboração ou `confirm_item` | active |
...
código — trocar (ex.: plugar dicionário de sinônimos no futuro) = nova config + reindex, zero
mudança de schema.
...
- `as_of` (date, opcional) — aplica a consulta (b); default = visão atual (a);
- `in_effect_only` (bool, default false) — exige `is_in_effect` (seção 5.4);
- `include_uncertain` (bool, default true — itens `uncertain` vêm **sinalizados**);
...
> é o que a busca semântica adiciona (seção 20). Buscar "Iniciativa Lunar" não encontra
> "Projeto Apollo": comportamento **esperado e testado** (cenário C11), não bug. A válvula da
> limitação é a curadoria (seção 10).
...
- **Back-fill futuro** (embeddings): reprocessamento é o mecanismo natural — seção 20.
...
- **Sem overlap.** Overlap mitiga perda de fronteira de *embeddings*, que não existem nesta fase;
  aqui só atrapalharia a proveniência (mesma frase em dois chunks = âncora ambígua) e o FTS
  (Postgres busca todos os chunks). Contexto de vizinhança vai no prompt (§9.3), não no banco.
...
A fase 1 é léxica — sinônimos, ambiguidades e conflitos **caem em filas por design**. A
curadoria não é acessório: é a válvula declarada da limitação léxica. Mono-usuário: o curador é
o dono; toda ação audita em `CurationAction` (sem coluna de ator — seção 2.3).
...
| `uncertain` | Aceitos provisórios aguardando corroboração | faixa 0.40–0.74 |
| `low_confidence` | Fragmentos `proposed` nunca consolidados | faixa < 0.40 |
...
conteúdo de documento é **dado**, nunca instrução. Transporte MCP local, mono-usuário (seção
2.3) — sem superfície de rede na fase 1.
...
### 14.3 Toolset `query` (determinístico; usado pela LLM orquestradora ou diretamente)
...
**`get_history`** — cadeia de versões pela linhagem.
...
### 14.4 Toolset `curation` (operador-dono; tudo audita em `CurationAction`)

```
list_review_queue   { kind?: "entity_match"|"disputed"|"uncertain"|"low_confidence",
                      limit?, offset? }
...
confirm_item        { item_kind, item_id }                       // uncertain → active
...
Carga inicial obrigatória (migração de seed). É o vocabulário concreto da fase 1 — novos tipos
...
**Premissas de volume (v1)** — corpus pessoal/organizacional:

| Métrica | Ordem de grandeza |
|---|---|
| `RawInformation` | ~10⁴ documentos |
| `RawChunk` / `InformationFragment` | ~10⁵–10⁶ |
| `KnowledgeNode` | ~10⁵ |
| `KnowledgeLink` + `NodeAttribute` | ~10⁶ |

Postgres single-instance atende com folga; nenhum sharding/replicação na fase 1.
...
**Observabilidade:** logs estruturados (JSON) e métricas por run — taxa de aceitação,
consolidações, `needs_review`, `disputed`, rejeições por camada de validação. Essas métricas são
o **insumo de calibração** dos thresholds (Apêndice A): revisão após os primeiros ~100
documentos.
...
intocado**, e o novo carrega `value = 2026-07-16` com o **mesmo `valid_from`** original.

**C7 — Point-in-time.** Dado C4; Quando consulto `deadline` com `as_of = 2026-06-15`; Então a
resposta é 15/07/2026 (consulta (b)); e a consulta (c) com `T` anterior à ingestão de C4
reproduz o que o sistema sabia então.
...
**C11 — Limitação léxica declarada.** Dado C1; Quando busco "Iniciativa Lunar"; Então **zero**
resultados do Apollo — comportamento esperado da fase 1, documentado (seção 7.3).
...
**C13 — Confiança baixa.** Quando a LLM propõe link com `confidence = 0.30`; Então nada
consolida; o fragmento fica `proposed` na fila `low_confidence`; o `ToolCall` registra
`rejected` com motivo de faixa.

**C14 — Corroboração.** Dado atributo `uncertain` (0.60); Quando fonte independente afirma o
mesmo valor; Então o atributo vira `active`, com proveniência das **duas** fontes.
...
7. Toda informação mutável é **bitemporal**, em links **e** atributos.
...
15. O sistema responde por **busca textual e grafo**, sempre citando a fonte — e declara
    abertamente o que **não** sabe responder nesta fase (sinônimos sem sobreposição léxica).
...
## 20. Caminho de evolução para embeddings

Embedding é **dado derivado** — se pendura no modelo sem reescrevê-lo. Plano, em ordem:

1. **Plugar um modelo de embedding** (local ou API; não exige LLM generativa).
2. **Preencher os campos reservados:** `RawChunk.embedding` e `KnowledgeNode.embedding`, via
   **back-fill por reprocessamento** (seção 8) — a verdade bruta não é tocada.
3. **Adicionar `pgvector` + índice ANN ao lado** dos índices full-text — nada é removido.
4. **Ligar o 3º modo de consulta:** busca semântica coexiste com full-text e grafo (busca
   híbrida) — entra como nova camada no pipeline da seção 7.2, com peso próprio.
5. **Ativar o sinal vetorial na resolução de entidade:** entra pela **interface de estratégia
   de similaridade** (seção 4.2) como mais um sinal — o fluxo de decisão (4.3) não muda. Ganho
   direto: sinônimos sem sobreposição léxica que hoje caem em `needs_review`.

**Por que não quebra nada:** nenhum campo muda de tipo/semântica (só se preenche o reservado);
full-text e grafo continuam; proveniência, bitemporalidade, linhagem e validação são ortogonais;
a reingestão idempotente funde o back-fill em vez de duplicar.

**Terreno já preparado na fase 1:** colunas `embedding` nuláveis; interface de similaridade;
`chunking_version`.
...
| # | Decisão | Valor |
|---|---|---|
| A1 | Banco / store único | PostgreSQL **17** — nenhum outro serviço de dados |
| A2 | Full-text | `tsvector` + GIN; configs `pt_unaccent_v1` (prosa) e `simple_unaccent_v1` (nomes), versionadas |
| A3 | Fuzzy léxico (resolução de entidade) | `pg_trgm`, **sinal único** na v1, atrás de interface; Levenshtein documentado como extensão |
| A4 | Sinônimos no índice | **Fora da v1**; porta preparada via config versionada |
| A5 | Blob do original | Inline (`content`); `storage_ref` reservado nulável |
| A6 | Acesso a dados | Query builder (Kysely ou Drizzle) + migrações SQL puras |
...
| A20 | Operação | **Mono-usuário, sem entidade `User`**; auditoria sem coluna de ator; multiusuário futuro é aditivo |
...
## Apêndice B — Changelog v4 → v5

| Mudança | Onde na v5 |
|---|---|
| Fórmula histórica corrigida (`superseded_at IS NULL`) + consulta de auditoria (c) adicionada | 5.3 |
| Convenção de intervalo semiaberto declarada | 5.2 |
| `InformationFragment.text_search` adicionado (promessa da §7 da v4 sem índice) | 3.2 |
| `KnowledgeNode.merged_into_node_id` adicionado (citado na §4 da v4, ausente do schema) | 3.3, 4.4 |
| `LinkTypeRule.cardinality` removido (sobreposição com `allows_multiple_current`) | 3.4 |
| `inactive` deixa de ser gravado → `effective_status` derivado | 5.4, 6.4 |
| Fluxo de **Correção** (retroativa) adicionado ao lado de Sucessão e Conflito | 5.6, 6.5-B |
| `aliases`/`name_search` normalizados para tabela `NodeAlias` | 3.3 |
| Registro `AttributeKey` criado (chaves de atributo governadas) | 3.4 |
| `EntityMatchReview`, `CurationAction`, `ComplianceDeletion` criados | 3.5 |
| `valid_from_source` + cadeia de justificativa de datas | 3.3, 6.5 |
| Guarda de duplicata + consolidação por re-afirmação + corroboração | 6.5 |
| `is_in_effect` para fatos futuros | 5.4 |
| Máquinas de estado explícitas | 6.6 |
| Pipeline de consulta com pesos/dedup/expansão definido | 7.2 |
| Fluxo de ingestão, transacionalidade e concorrência | 9 |
| Processo de chunking detalhado (5 estágios, algoritmo greedy, convenção de offset) | 9.2, A22 |
| Protocolo de extração de fragmentos (contexto, ordem, sub-regras, identidade via resolver) | 9.3 |
| `RawChunk.locator` para citação granular (página/linha/locutor) | 3.1, A23 |
| Curadoria: filas e operações | 10 |
| Catálogo MCP (3 toolsets, 17 ferramentas) | 14 |
| Catálogos seed (8 NodeTypes, 10 LinkTypes, 10 AttributeKeys) | 15 |
| NFRs e cenários de aceitação (C1–C15) | 16, 17 |
| Todas as decisões "ex.:" fechadas em ADRs | Apêndice A |
| Mono-usuário sem entidade `User` | 2.3, A20 |
