...
Envelope comum de resposta (o mesmo formato é devolvido pelas rotas REST equivalentes):
...
| **BFF formalizado** (Fastify): REST (SPA) + MCP (LLM) como **dois transportes sobre uma única camada de serviço/validação**; `query`/`curation` espelhados em REST, `ingest` permanece MCP-only | 2, 14, A28 |
