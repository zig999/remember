...
> MCP; PostgreSQL 17 via Supabase Cloud com driver `pg` raw; Supabase Auth com JWT no
> middleware; RLS desligado). **Não use este arquivo para desenvolvimento** — mantido apenas
> como registro histórico.

# Sistema de Segundo Cérebro — Modelagem v6 (Grafo + Full-text, léxico, mono-usuário — especificação fechada)
...
>
> **Projeto pessoal, mono-usuário por especificação** (seção 2.3 e A20). Não há multiusuário,
> autenticação de pessoas nem autorização por papel; o operador é o **dono**.
>
> A v6 é a v5 após três ajustes deliberados de relação custo × benefício, todos registrados
> aqui e no changelog (Apêndice B):
>
> 1. **Embeddings/busca vetorial são um não-objetivo permanente** (seção 20, A24). A recuperação
>    é **léxica + grafo**, e assim permanece. Não há "fase 2" de embeddings; as colunas vetoriais
>    da v5 foram **removidas** (não apenas reservadas). A consequência — sinônimos sem
>    sobreposição léxica não casam automaticamente — é **assumida como permanente**, e a curadoria
>    é a sua válvula (seções 4.5, 7.3, 10, 20).
...
> 3. **Duas filas de curadoria, não quatro.** Restam `entity_match` e `disputed` — as que pagam a
>    conta. `uncertain` e `low_confidence` deixam de ter fila dedicada e passam a ser **flags de
>    exibição** (sinalizados nos resultados, promovidos por corroboração automática, tratados
>    ad-hoc). Filas dedicadas para eles ficam diferidas até o volume justificar (seções 10, A26).
>
...
### 2.3 Operação mono-usuário (projeto pessoal)

O sistema é **mono-usuário por especificação — um projeto pessoal**. Não existe entidade `User`:
...
- Multiusuário futuro, se algum dia existir, é **aditivo**: acrescenta-se entidade de ator e
  coluna de autoria às trilhas existentes, sem alterar a semântica de nada. Não é objetivo desta
  especificação.
...
- **Por que diferida.** A reconstrução "o que o sistema sabia em T" é auditoria forense de alto
  ...
  mono-usuário. Não é construída, testada nem mantida agora.
...
permanente** da escolha léxica. Mono-usuário: o curador é o dono; toda ação audita em
...
É essa distinção que reduz a curadoria de quatro filas (v5) para **duas** (v6) **sem violar o
princípio**: a v6 mantém os vetos e relaxa a *consequência*. `entity_match` e `disputed` só se
resolvem com julgamento humano — são fila de verdade. `uncertain` tem **saída automática**
(corroboração promove `uncertain → active`, §6.5) e `low_confidence` é ruído de baixa prioridade
que fica `proposed` e sinalizado — ambos preservados, ambos visíveis, **nenhum** descartado ou
chutado, e **nenhum** exigindo uma "caixa de entrada" a zerar.
...
> dono**. O banco é detalhe de implementação. É por isso que a v6 economiza no recurso caro
...
- **Não existem colunas de embedding.** As colunas `embedding` que versões anteriores reservavam
  em `RawChunk` e `KnowledgeNode` foram **removidas**. Não se carrega peso de schema por uma
  capability que não será usada.
...
| A20 | Operação | **Mono-usuário, projeto pessoal, sem entidade `User`**; auditoria sem coluna de ator; multiusuário não é objetivo |
...
## Apêndice B — Changelog v5 → v6

| Mudança | Onde na v6 |
|---|---|
| **Embeddings tornam-se não-objetivo permanente** (era "fase 2"); seção de evolução vetorial removida e substituída por "Não-objetivos e limites permanentes" | 20, A24 |
| Colunas `embedding` de `RawChunk` e `KnowledgeNode` **removidas** (eram "reservadas") | 3.1, 3.3 |
| Linha "Vetores (futuro)/pgvector" da tabela de plataforma vira "Embeddings/vetores — fora de escopo permanente" | 2.2 |
| Interface de similaridade reformulada: extensão **léxica** (Levenshtein), sem sinal vetorial | 4.2, A3 |
| Limitação léxica reescrita de "fase 1 / melhora com embeddings" para **permanente, válvula = curadoria** | 4.5, 7.3, 11→10, C11 |
| Consulta (c) "o que o sistema sabia em T" **diferida**: `recorded_at` preservado, caminho não construído | 5.3, 20.2, A25 |
| `superseded_at` mantido e ativo para **correção** (6.5-B), linhagem e versão corrente | 5.6, 6.5-B |
| `get_history` destacado como visibilidade de linhagem independente da consulta (c) | 5.3, 14.3 |
| Filas de curadoria reduzidas de **4 para 2** (`entity_match`, `disputed`) | 10.1, 14.4, A26 |
| `uncertain` / `low_confidence` viram **flags de exibição** (sem fila); corroboração automática é a saída de `uncertain` | 6.4, 6.5, 6.6, 7.3, 10.1 |
| Fundamento da curadoria explicitado: os vetos de §1 (não chutar / não descartar em silêncio) **não obrigam fila humana** — daí **quatro destinos** (chutar/descartar proibidos; fila exige humano; flag preserva sem exigir), o que justifica 2 filas sem violar o princípio | 10 (preâmbulo) |
| **Dimensionamento realinhado ao projeto pessoal enxuto** (centenas de documentos): nós ~10²–10³, links/atributos ~10³–10⁴ — 2–3 ordens abaixo de um corpus organizacional; índices viram conforto (não *load-bearing*), calibração (A21) e promoção de filas (A26) tornam-se oportunísticas/improváveis; gargalo é ingestão-LLM + curadoria, não o banco | 16 |
| `list_review_queue.kind` reduzido a `entity_match`/`disputed`; flag `low_confidence` adicionada em `search` | 14.3, 14.4 |
| Cenário C7 ajustado (testa só valid-time (b); (c) marcado como diferido); C13 reescrito (flag, não fila) | 17 |
| Mono-usuário reforçado como **projeto pessoal**; multiusuário deixa de ser "futuro aditivo" e vira não-objetivo | 2.3, A20 |
| Bullet de back-fill de embeddings removido de Idempotência; nota de overlap reescrita | 8, 9.2 |
| Princípios 7 e 15 reescritos (transação diferida; limite léxico permanente) | 18 |
