...
describe("chat-agent.service.runTurn", () => {
  ...
  // -------------------------------------------------------------------------
  // TC-04 v2.8 — seam-removal regression
  //   The chat-side `start_async_ingestion` dispatcher seam was retired.
  //   `ingest_directed` flows through the SAME standard catalog dispatch path
  //   as any query tool. The legacy BR-43/BR-45 special-dispatch tests below
  //   were deleted; this single regression test pins the new invariant:
  //   `ingest_directed.handler` is invoked DIRECTLY by the agentic loop with
  //   the model's input, no chat-side wrapper in front of it.
  // -------------------------------------------------------------------------

  it("TC-04 / BR-43 v2.8: ingest_directed routes through the standard catalog handler (no chat-side dispatcher)", async () => {
    // Register `ingest_directed` on the `ingest` toolset with a handler that
    // records the input. After TC-04 the chat agentic loop must invoke this
    // handler DIRECTLY — there is no special-case branch, no ingestDispatcher
    // injection. A regression that resurrected the seam would either bypass
    // this handler entirely or wrap it.
    const mcp = buildMcpWithAllChatTools();
    const directedInvocations: unknown[] = [];
    mcp.registerTool("ingest", {
      name: "ingest_directed",
      description: "stub ingest_directed (catalog handler)",
      inputSchema: z.object({}).passthrough(),
      handler: async (input: unknown) => {
        directedInvocations.push(input);
        return {
          ok: true,
          result: {
            outcome: "ingested" as const,
            raw_information_id: "00000000-0000-4000-8000-000000000002",
            llm_run_id: "00000000-0000-4000-8000-000000000003",
          },
        };
      },
    });
    ...
    const catalog = buildChatToolCatalog(
      mcp,
      buildEnv({ CHAT_INGEST_ENABLED: true } as Partial<Env>)
    );
    if (catalog === undefined) throw new Error("14-tool catalog should resolve");
    ...
    // The catalog handler was invoked with the model's input verbatim — no
    // adapter shim transformed it.
    expect(directedInvocations).toEqual([
      { source_type: "note", content: "hello" },
    ]);
    const toolResult = events.find((e) => e.type === "tool_result");
    if (toolResult === undefined || toolResult.type !== "tool_result") {
      throw new Error("no tool_result emitted");
    }
    expect(toolResult.tool).toBe("ingest_directed");
    expect(toolResult.ok).toBe(true);
    expect(toolResult.result).toMatchObject({ outcome: "ingested" });
    // Terminal frame is reached — the loop continues after the catalog call.
    const terminal = events[events.length - 1];
