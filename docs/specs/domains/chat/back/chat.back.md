...
> Stack: Node.js 20 LTS + TypeScript strict + Fastify | DB: PostgreSQL 17 (Neon) — owns 4 tables (`chat_conversation`, `chat_message`, `chat_tool_call`, `chat_graph_view`) + 1 enum (`chat_message_role`) via migrations `0004_chat_persistence.sql` + `0005_chat_graph_view.sql` | Version: 2.6.0 | Status: draft | Layer: permanent
