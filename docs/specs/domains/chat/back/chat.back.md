...
> Stack: Node.js 20 LTS + TypeScript strict + Fastify | DB: PostgreSQL 17 (Neon) — owns 4 tables (`chat_conversation`, `chat_message`, `chat_tool_call`, `chat_graph_view`) + 1 enum (`chat_message_role`) via migrations `0004_chat_persistence.sql` + `0005_chat_graph_view.sql` | Version: 2.11.0 | Status: draft | Layer: permanent
> Business spec: `../chat.spec.md` (v2.9.0)
> REST contract: `../openapi.yaml` (v2.9.0)
...
> **v2.11 additive deviation (Graph projection now covers directed ingestion — reconciles the "no graph_delta for ingestion tools" invariant).** Owner-approved 2026-07-14. Adopts `chat.spec.md` v2.9.0 + `openapi.yaml` v2.9.0. Root motivation: today `GraphSpace` renders the right-column knowledge graph only when the chat LLM invokes one of the FOUR read-only graph-producing tools (`traverse`, `get_node`, `list_nodes`, `search`) — the four literals of `GRAPH_TOOL_NAMES` in `backend/src/modules/chat/service/graph-normalizer.ts:103` and the emission gate in `conversations.routes.ts:972-988`. `ingest_directed` (BR-43 v2.8) is classified as a `non-graph tool` (BR-41 v2.1 trigger set exclusion, made explicit in the header note above and in `chat.spec.md` §12 out-of-scope until v2.8.2). Consequence: when the Owner types "crie o João e ligue-o à Rede Unifique", the BFF persists the nodes + links correctly (confirmed live: 9 nodes + 8 links in `knowledge_node` / `knowledge_link`) but the SPA's `GraphSpace` STAYS EMPTY until the Owner explicitly asks a follow-up query. The follow-up recipe (`get_node` on ids from `result.run.affected_nodes`; BR-18 v4 block 4C "post-ingestion playbook") IS operational but requires an extra Owner turn — a UX gap the chat-graphspace was supposed to close. Under v2.11, `ingest_directed` becomes a fifth graph-producing tool for `graph_delta` emission: the same route-owned projector that runs today after `traverse` / `get_node` / `list_nodes` / `search` `tool_result` events now ALSO runs after a successful `ingest_directed` `tool_result`. The data the projector needs is already in the envelope: `result.run.affected_nodes` (each entry `{id, canonical_name, node_type}` — see `ingestion.back.md` BR-33 / `AffectedNodeOutput` schema) supplies the nodes; `result.report[]` supplies the links (entries with `kind === "link"` whose `outcome` is in the accepted family — `accepted` / `consolidated` / `superseded_previous` / `needs_review` / `uncertain` / `disputed`; each carries the persisted `link_id` and the `link_type` slug from the input payload, and the endpoints are resolved by pairing `source_ref` / `target_ref` against the accepted `kind === "node"` entries of the same `report[]`). Nothing new to compute server-side: the directed orchestrator ALREADY produces both surfaces synchronously (`ingestion.back.md` BR-34); the chat route just projects them onto the existing wire (`GraphDeltaWire { source_tool, nodes[], links[] }`, BR-41 §1.2). Wire changes: (a) `GraphDeltaEvent.source_tool` gains the fifth literal `"ingest_directed"` (openapi.yaml v2.9.0 additive minor); (b) frame ordering invariant #3 extended to include `ingest_directed`; (c) new per-tool projection rule for `ingest_directed` in the semantics section — `nodes[]` from `run.affected_nodes` with `status: "active"` (directed items are stated-by-construction: server forces `confidence = 1.0` and `valid_from_basis: "stated"`, BR-43 v2.8, so on creation they cannot land in `needs_review` / `merged` / `deleted` — those statuses arise from resolution paths or compliance actions the directed path does not trigger); `links[]` from `report[]` entries in the accepted family (`is_temporal` and optional `link_type_label` resolved via the SAME `CatalogSnapshot.linkTypeByName` lookup used by the `traverse` projection — catalog miss -> `is_temporal: false`, `link_type_label` OMITTED; `is_in_effect`/`status`/`flags` OMITTED on the directed path — the freshly persisted link carries no view-derived `is_in_effect` yet, the SPA renders it with the neutral default, and a follow-up `traverse` surfaces those fields on a later turn if the Owner asks for them). Domain BR changes: **BR-41 v2.11** — the trigger set grows to five names (`{traverse, get_node, list_nodes, search, ingest_directed}`); a fifth normalization arm is added to `service/graph-normalizer.ts` (`normalizeIngestDirected(result, catalog): GraphDeltaWire | null` — pure, no I/O; the fifth arm does NOT need `client?` because the ingest_directed envelope already carries `canonical_name`/`node_type` on `run.affected_nodes` and `link_id` on `report[]` — no hydration needed, unlike `search`); frame ordering, defensive `try/catch` WARN behaviour, non-persistence, and non-replay stay UNCHANGED. **BR-43 v2.11** — the "Sequence inside the agentic loop" step ceases to assert "NO `graph_delta` emission"; instead it points at BR-41 v2.11 for the projection. No other BR renumbering. §1.1 `graph-normalizer.ts` blurb extended with the fifth arm. §1.2 `ChatEvent.graph_delta` wire is UNCHANGED (the `source_tool` field is already typed as `string`; the fifth literal does not require a new type). §7 External Integrations: NO change — the projector still consumes only `CatalogSnapshot.linkTypeByName` (already forwarded on `ChatRouteDeps`, BR-41); no new value/type imports from `ingestion` or `knowledge-graph`. §9 Observability: the existing WARN log shape `chat.graph_delta_normalize_failure` now ALSO covers the `ingest_directed` arm (no new log shape needed — the normalizer signature is uniform); the existing counter `chat_ingest_directed_total{ok}` is unaffected (the `graph_delta` emission is orthogonal — it does NOT bump the ingest counter). §12 Known Technical Constraints: the existing entry about `graph_delta` requiring the catalog snapshot is extended in-place to reference the `ingest_directed` arm; a new bullet documents that a partially-rejected `ingest_directed` payload emits a graph_delta covering ONLY the accepted items (the dropped rejected/error/dependency_failed items are surfaced in the text channel via `report[i].status`, not in the graph). §13 Out of Scope: the v2.8 bullet "no graph_delta for ingestion tools" (implicit at v2.1 landing; made explicit in v2.9 openapi header prose) is REVOKED; the `ingest_directed` path now feeds the graph on the same turn. Rationale for surgery over a bigger refactor: the entire delta stays inside `service/graph-normalizer.ts` (dispatcher gets a fifth arm) + `routes/conversations.routes.ts` (the emission gate gets a fifth allowed tool name); the ChatEvent union already carries `graph_delta` via a route-owned synthesis (BR-41 v2.1) — no agent-side seam moves; no new SSE frame; no new error code; no new env var; no migration; no schema change. Backwards-compatibility: clients written against v2.1..v2.10 that already handle `graph_delta` transparently (dedup by node `id`) receive the new payloads correctly — the fifth `source_tool` is discriminator-friendly; clients that explicitly switch on `source_tool` and enumerate the four legacy literals will drop the new frame silently (safe fallback: the SPA is expected to add the fifth literal to its handler; until it does, the graph stays empty on ingest turns — same UX as v2.10, no regression). PRESERVED from v2.10.1: P2.1 canonical error-code mapping (§10 / §7 / §9 / BR-06 / BR-43); from v2.9: temporal & memory fidelity (BR-31 v2.9 / BR-33 v2.9 / BR-46 / BR-47); from v2.8: directed-ingestion catalog (BR-05 / BR-43 v2.8 / BR-44) — only BR-43's "Sequence inside the agentic loop" prose is amended; from v2.7: graph-view snapshot v2 (BR-42); from v2.6: per-tool JSON Schema descriptors (BR-06 step 7); from v2.5: ontology-aware prompt (BR-18 v4); from v2.3: graph-view snapshot persistence; from v2.2: multi-row persistence (BR-29 / BR-31 / BR-32); from v2.1: `graph_delta` projection (BR-41 — trigger set widens; wire, defensive gates, non-persistence, non-replay all UNCHANGED). Reconciles with `chat.spec.md` v2.9.0 BR-43 v2.6 step 9 + openapi.yaml v2.9.0 `GraphDeltaEvent.source_tool` fifth-literal extension.
...
backend/src/modules/chat/
  ...
  service/
    ...
    graph-normalizer.ts        # v2.1 (NEW), extended v2.11: pure projection
                               #   from a graph-producing tool_result envelope
                               #   into GraphDeltaWire ({source_tool, nodes[],
                               #   links[]}). Dispatches by tool name — the
                               #   trigger set (GRAPH_TOOL_NAMES) is FIVE names
                               #   under v2.11 (was four under v2.1..v2.10):
                               ...
                               #     - ingest_directed (v2.11, NEW): projects
                               #       nodes from `result.run.affected_nodes`
                               #       (each entry already carries
                               #       {id, canonical_name, node_type} —
                               #       `ingestion.back.md` BR-33 /
                               #       `AffectedNodeOutput`; projector sets
                               #       `status: "active"` — directed items are
                               #       stated-by-construction, BR-43 v2.8);
                               #       projects links from `result.report[]`
                               #       entries whose `kind === "link"` AND
                               #       `outcome` is in the accepted family
                               #       (`accepted`/`consolidated`/
                               #       `superseded_previous`/`needs_review`/
                               #       `uncertain`/`disputed`); entries with
                               #       `outcome` in
                               #       {rejected, error, dependency_failed}
                               #       are DROPPED; each surviving entry pairs
                               #       its persisted `link_id` with the input
                               #       `link_type` slug and resolves endpoints
                               #       (`source_node_id`/`target_node_id`) via
                               #       the accepted `kind === "node"` entries
                               #       of the SAME `report[]`
                               #       (`node_ref -> node_id` map). No I/O —
                               #       the ingest_directed envelope already
                               #       supplies every field the projector
                               #       needs; `client?` is NOT threaded into
                               #       this arm (unlike `search`).
...
6. (in-loop) Consume `chatAgentService.runTurn(...)`. The route layer is responsible for assembling the iteration-by-iteration persistence sequence in tandem with the SSE drain. Per iteration `i`:
   ...
   c. On each `ChatEvent.tool_result{tool_use_id, tool, ok, arguments, result, is_error, error_message, duration_ms}`:
      ...
      - (v2.1; v2.11 widens the trigger set to five names) If `evt.tool` is one of `{traverse, get_node, list_nodes, search, ingest_directed}` AND `ok===true` AND a `CatalogSnapshot` is available, synthesise + write the `graph_delta` frame (BR-41 v2.11).
...
### BR-41 -- `graph_delta` SSE frame projection (Chat-Graph) — v2.11 trigger set = 5 tools
**Related UC:** UC-02 (variant: a graph-producing tool is invoked) — referenced as **UC-CG-01..UC-CG-04** + **UC-CG-14** (v2.11 — Load subgraph from an ingest_directed command) in the front-side feature plan.
...
1. **Trigger (v2.11 widens the set from 4 to 5 names).** The drain loop receives a `ChatEvent.tool_result` with `ok === true`. If `evt.tool` is one of `{traverse, get_node, list_nodes, search, ingest_directed}` AND a `CatalogSnapshot` is available on `ChatRouteDeps`, the route invokes `normalizeToolResult(evt.tool, evt.result, catalog, client?)` from `service/graph-normalizer.ts`. Failed tool calls (`ok === false`) NEVER produce a `graph_delta` — by construction, `evt.result` is `null` on failure and the projector is gated behind `evt.ok` to avoid surfacing a misleading "graph data arrived" signal (consistent with `chat-graphspace-plan.md` §8.2 EV-CG-03 / UC-CG-06). The `ingest_directed` arm is v2.11-additive; the underlying implementation constant `GRAPH_TOOL_NAMES` in `backend/src/modules/chat/service/graph-normalizer.ts:103` gains the fifth literal, and the route-side emission gate in `conversations.routes.ts` (today at lines 972-988) admits the fifth name — no other seam changes.
2. **Normalization (per tool — v2.11 adds the fifth arm).**
   ...
   - `ingest_directed` (v2.11, NEW — no SQL, no hydration). The envelope shape is `{ outcome: "ingested", raw_information_id, llm_run_id, chunk_count, run: { ..., affected_nodes?: Array<{id, canonical_name, node_type}> }, report: ItemReport[], summary: {...} }` (front-half contract in BR-43 v2.8; back-half schema in `ingestion.back.md` BR-33 / BR-34 / `AffectedNodeOutput`). The projector:
     - **Nodes.** `nodes = (result.run.affected_nodes ?? []).map(n => ({ id: n.id, node_type: n.node_type, canonical_name: n.canonical_name, status: "active" }))`. `status` is forced to `"active"` on EVERY node — directed items are stated-by-construction: the server forces `confidence = 1.0` AND `valid_from_basis: "stated"` on every dispatched `propose_*` (BR-43 v2.8), so a freshly created / re-affirmed directed node cannot land in `needs_review` / `merged` / `deleted` (those statuses arise from resolution paths or compliance actions that the directed path does not trigger). If `affected_nodes` is absent (contract soft-optionality, `ingestion.back.md` BR-33) OR empty, `nodes = []` — the frame still emits (possibly with `links[]` still non-empty; and even a `{nodes:[], links:[]}` empty delta is contractual under the v2.11 header — no early return here, unlike `search`).
     - **Links.** Build the `node_ref -> node_id` map from the accepted `kind === "node"` entries of the same `report[]` (accepted family = `resolution` in `{accepted, consolidated, superseded_previous, needs_review, uncertain, disputed}` — same predicate as the link-side filter, applied to the node-side entries). Then `links = report.filter(r => r.kind === "link" && ACCEPTED.has(r.outcome)).map(r => buildLinkWire(r, nodeRefIndex, catalog))`, where `ACCEPTED = new Set(["accepted", "consolidated", "superseded_previous", "needs_review", "uncertain", "disputed"])`. Entries with `outcome` in `{rejected, error, dependency_failed}` are DROPPED — the persisted link either does not exist or is not surfaced (the natural-language report already carries that outcome in the text channel, so the graph stays truthful and does not misrepresent a partial failure).
     - **Endpoint resolution.** Each surviving link report entry carries `source_ref` and `target_ref` (the local refs from the original payload; `ingestion.back.md` BR-34 step 5 links branch). `buildLinkWire` looks them up in the `node_ref -> node_id` map built above. If EITHER endpoint is unresolved (edge case — e.g. a link somehow persisted while one endpoint was `rejected`, a race the directed contract does not currently reach), the link is DROPPED silently (WARN log; no throw). Each `link_id` on the report entry is the persisted uuid; the projector composes `GraphLinkWire.id = link_id`, `.link_type = <input payload's link_type slug for this ref pair>` (from the same `report[]` entry — the orchestrator surfaces the resolved slug on each item; `ingestion.back.md` BR-33 `LinkReportItem`).
     - **Catalog fields.** `is_temporal` and optional `link_type_label` are resolved by looking up `link_type` in `CatalogSnapshot.linkTypeByName` — SAME lookup used by the `traverse` arm; catalog miss -> `is_temporal: false` (defensive default), `link_type_label` OMITTED.
     - **Fields omitted on the directed path.** `is_in_effect`, `status` (assertion_status), and `flags` are OMITTED from every `GraphLinkWire` emitted by this arm. Rationale: `is_in_effect` is a view-derived boolean computed from temporal windows over `knowledge_link_resolved` (§5.4) and requires a read query the directed path does NOT issue synchronously; `status` and `flags` similarly reflect view-derived assertion state. The SPA renders directed links with the neutral default (solid/temporal-aware based on `is_temporal`), and a follow-up `traverse` on a subsequent turn surfaces those fields if the Owner asks. This mirrors the openapi.yaml v2.9.0 "graph_delta semantics" section for the ingest_directed arm.
     - **Payload sizing.** An `ingest_directed` invocation is bounded by the BFF's per-call ceilings (`ingestion.back.md` BR-34 payload caps: 10 fragments / 10 nodes / 10 attributes / 10 links per call); the projected `nodes[]` + `links[]` are therefore small (≤ 10 each). No additional caps are introduced by the projector.
     - **Signature.** The fifth arm is a PURE function: `normalizeIngestDirected(result: unknown, catalog: CatalogSnapshot): GraphDeltaWire | null`. It does NOT receive `client?` — no DB hydration is needed (unlike `search` G-A) because the envelope already carries `canonical_name` + `node_type` on `affected_nodes` and `link_id` on `report[]`. The chat module's boundary rule (§1.1 note) does NOT widen further under v2.11: the projector consumes ONLY `CatalogSnapshot` (already permitted since v2.1) — no new imports from `ingestion/` or elsewhere; the envelope shape it reads is documented by `ingestion.back.md` BR-33 / BR-34 but chat does not import that TypeScript type at value-time (a shared `IngestDirectedResult` type CAN be imported from `ingestion/mcp/mcp-schemas.ts` at TYPE-only level via `import type` — same category of read-only cross-module coupling already permitted for `CatalogSnapshot`). Rationale: the projector is a routing / shape-mapping function; treating the envelope as `unknown` and validating field-by-field would duplicate the Zod schema `ingestion` already owns.
3. **Wire emission (unchanged wire; trigger set widens under v2.11).** The projector returns `GraphDeltaWire | null`. When non-null AND the catalog snapshot is available, the route synthesises a `ChatEvent.graph_delta` and writes it through `projectSseFrame` as `event: graph_delta\ndata: <JSON>\n\n`. Frame ordering is contractual: the `graph_delta` ALWAYS follows its originating `tool_result` in the SAME drain-loop iteration, for EACH of the five graph-producing tool names (four read tools + `ingest_directed`, v2.11). When the catalog snapshot is absent (degraded mode — e.g. boot raced ahead of the catalog), the path is silently skipped for every tool (including `ingest_directed`); `tool_result` still emits normally. The `source_tool` discriminant on the emitted frame is the same string as `evt.tool` — clients that switch on it MUST accept the fifth literal `"ingest_directed"` under v2.11 (openapi.yaml v2.9.0 additive).
...
5. **Persistence (unchanged under v2.11).** `graph_delta` is NOT persisted to `chat_tool_call` — for ANY of the five source tools, including `ingest_directed`. The audit trail for the underlying tool invocation lives on the originating `chat_tool_call` row inserted in step 6 of BR-29 (`arguments` = the structured `ingest_directed` payload, `result` = the full per-item report + `run.affected_nodes`, per BR-32 / BR-43 v2.8 step 5) — BR-32 is the single source of truth for tool-call persistence. Re-running the same conversation cannot reproduce the `graph_delta` (no replay path for it, per step 6 below). Refresh requires re-issuing a graph-producing tool call — either a read (`get_node` / `traverse` on the ids the SPA already has) or another directed ingestion (which would create a fresh run, per §18 re-affirmation semantics).
6. **Idempotent replay (UC-07) — unchanged under v2.11.** The replay path described in BR-27 emits `llm_start{1}` + `text_delta(<stored>)` + `done{stored}` and closes — NO `tool_result`, NO `graph_delta`, regardless of whether the original turn invoked read tools OR `ingest_directed`. The SPA's `useGraphStore` is responsible for ignoring the replay path (no tool_start signal), per `chat-graphspace-plan.md` §8 sequence. This intentionally preserves side-effect containment: re-running `ingest_directed` on replay would create a NEW `RawInformation` (the timestamp/nonce keeps `content_hash` unique — `ingestion.back.md` BR-34 step 2) and would re-consolidate the same nodes/links, growing provenance without changing the visible graph (§18); the replay skipping the `graph_delta` matches the replay skipping the underlying dispatch.
...
- Route writes the SSE frames, persists the `chat_tool_call` audit row (BR-32), AND — on `ok === true` AND a `CatalogSnapshot` available on `ChatRouteDeps` (v2.11) — synthesises the `graph_delta` frame IMMEDIATELY AFTER the `tool_result` and BEFORE any subsequent `llm_start` / `text_delta` / `tool_start` of the next iteration. See **BR-41 v2.11** for the full projection contract (nodes from `run.affected_nodes` with `status: "active"`; links from `report[]` accepted-family entries; catalog miss -> `is_temporal: false`; defensive WARN-and-skip on exception; not persisted, not replayed). Rationale for the change: this closes the UX gap where `ingest_directed` persists the graph correctly but the SPA `GraphSpace` stays empty until the Owner triggers a follow-up read. The follow-up recipe on `result.run.affected_nodes` (BR-18 v4 block 4C post-ingestion playbook) IS still valid — it lets the model narrate the ingestion result in the text channel — but it is no longer required for the graph to render.
...
- **`graph_delta` requires the catalog snapshot (v2.1, BR-41; scope widened to `ingest_directed` in v2.11).** The route reads the `CatalogSnapshot` from `ChatRouteDeps` (forwarded by `app.ts` at boot). When the snapshot is unavailable (degraded mode — e.g. boot raced ahead of the catalog loader), the route silently SKIPS `graph_delta` emission — for ALL FIVE graph-producing tools including `ingest_directed` — while keeping `tool_result` intact. This is a degraded UX, NOT a turn failure. There is no automatic recovery path other than restart; the BFF should log ERROR at boot if the catalog fails to load (existing `knowledge-graph` invariant).
- **`graph_delta` is not persisted; not replayable (v2.1; still true for `ingest_directed` under v2.11).** Per BR-41 step 5, the frame is observational only — the audit trail for the originating tool call lives on the existing `chat_tool_call` row (BR-32). The idempotent-replay path (UC-07, BR-27) does NOT re-emit `graph_delta`; clients reconstructing the visual graph from a replay must re-issue a graph-producing tool call (read tool OR another `ingest_directed` — the latter would create a fresh run per §18 re-affirmation semantics, so replays typically use reads). Under v2.11 this rule applies uniformly to all five source tools.
- **`graph_delta` on `ingest_directed` reflects the ACCEPTED subset of the payload (v2.11, BR-41 step 2 ingest_directed arm).** When an `ingest_directed` payload is partially rejected (`report[i].status` in `{rejected, error, dependency_failed}` for some items), the `graph_delta` frame contains ONLY the accepted family — no dropped items appear in `nodes[]` or `links[]`. This is by design: the graph MUST NOT display links or nodes that do not exist in the persisted state. The rejected items are surfaced in the text channel via the model's natural-language summary of `report[]` — the Owner sees WHY an item was rejected in the assistant text, and sees WHAT was ingested in the graph. If EVERY item in a payload is rejected (a valid but pathological case — every `LinkTypeRule` in the payload violated the ontology, for instance), the projector emits `{nodes:[], links:[]}` (an empty delta — the frame still emits because the invariant `source_tool + ok=true -> one graph_delta` holds; the SPA is expected to render the empty delta as a no-op).
- **Envelope shape coupling with `ingestion` (v2.11, BR-41 ingest_directed arm).** The projector reads three fields from the `ingest_directed` envelope: `result.run.affected_nodes`, `result.report[]` (with fields `kind` / `outcome` / `link_id` / `source_ref` / `target_ref` / `link_type` on link entries; `node_ref` / `resolution` on node entries), and NOTHING ELSE. The chat module treats the envelope as `unknown` at value-time and destructures defensively (missing / malformed fields -> the projector returns a minimal delta or `null` and logs WARN `chat.graph_delta_normalize_failure`); the TypeScript shape MAY be pulled via `import type` from `ingestion/mcp/mcp-schemas.ts` (type-only cross-module coupling, same category as `CatalogSnapshot`). A future change to the ingest_directed envelope shape (e.g. renaming `affected_nodes` to `touched_nodes`) MUST update both the type import (or the raw field name in the destructuring) AND the projector; the coupling is documented here so that a lone `ingestion.back.md` refactor cannot silently break the chat graph.
...
- **REVOKED in v2.11 — "no `graph_delta` for ingestion tools".** The v2.1 trigger set restricted `graph_delta` emission to the four read-only graph-producing query tools (`traverse` / `get_node` / `list_nodes` / `search`); the v2.4/v2.8 async / directed ingestion additions did NOT feed the graph on the same turn. Under v2.11, `ingest_directed` joins the trigger set (BR-41 v2.11, per-tool projection rule for the fifth arm); the exclusion is lifted. The four read-only tools remain graph-producing; `ingest_directed` is now a fifth graph-producing tool. The remaining ingestion tools that are NOT in the chat catalog (the four `propose_*` handlers, `ingest_document`, `get_ingestion_status`) STAY out of the trigger set — they are not reachable from chat under v2.8+ (`ingestion.back.md` BR-32 WITHDRAWN + BR-45 RETIRED). Any future write-bearing chat tool would be evaluated independently for graph-projection under the same criteria (does the envelope carry affected_nodes-equivalent + link report? if yes, add an arm; if no, remain non-graph).
...
| 2.11.0 | 2026-07-14 | Back Spec Agent | minor (additive, feature-flagged) | **`graph_delta` projection now covers `ingest_directed` (chat-graph coupling for directed ingestion).** Adopts `chat.spec.md` v2.9.0 + `openapi.yaml` v2.9.0. Revokes the v2.1 invariant that constrained the `graph_delta` trigger set to the four read-only graph-producing tools; `ingest_directed` (BR-43 v2.8) joins the trigger set as a fifth graph-producing tool. Root motivation: today a directional command like "crie o João e ligue à Rede Unifique" persists nodes + links correctly (confirmed live: 9 nodes + 8 links) but the SPA `GraphSpace` stays empty until the Owner issues a follow-up read. The data the projector needs is already in the `ingest_directed` envelope: `result.run.affected_nodes` supplies `{id, canonical_name, node_type}` for the nodes; `result.report[]` supplies `{link_id, source_ref, target_ref, link_type, kind, outcome}` for the links; catalog fields are resolved via the SAME `CatalogSnapshot.linkTypeByName` lookup already used by the `traverse` arm. Changes: (a) Header amended with the v2.11 additive-deviation paragraph documenting the motivation, wire additions (openapi.yaml v2.9.0 `source_tool` fifth literal), semantics, backwards-compatibility, and the surgery scope (dispatcher fifth arm + route emission gate — no seam moves elsewhere). Version bumped 2.10.1 → 2.11.0; business-spec ref bumped v2.8.2 → v2.9.0; openapi ref bumped v2.8.0 → v2.9.0. (b) §1.1 `graph-normalizer.ts` blurb extended with the fifth arm — pure, no I/O (unlike `search` G-A which hydrates); `client?` not threaded into this arm; boundary rule of §1.1 unchanged (only `CatalogSnapshot` consumed cross-module; `IngestDirectedResult` MAY be pulled as `import type` from `ingestion/mcp/mcp-schemas.ts`). `GRAPH_TOOL_NAMES` (implementation constant) grows to five literals. (c) BR-29 step 6.c: the `(v2.1)` gate annotation updated to `(v2.1; v2.11 widens the trigger set to five names)` — `evt.tool ∈ {traverse, get_node, list_nodes, search, ingest_directed}` AND `ok===true` AND catalog available -> synthesise `graph_delta`. (d) **BR-41 v2.11** — trigger set widened; a fifth per-tool normalization arm added: nodes from `run.affected_nodes` with `status: "active"` (directed items are stated-by-construction — `confidence=1.0`, `valid_from_basis:"stated"`, BR-43 v2.8); links from `report[]` entries in the ACCEPTED family (`accepted`/`consolidated`/`superseded_previous`/`needs_review`/`uncertain`/`disputed`); dropped families (`rejected`/`error`/`dependency_failed`) NEVER appear in the frame — the graph shows only what was persisted; endpoints resolved via the accepted `kind === "node"` entries of the same `report[]` (`node_ref -> node_id` map built inline); `is_temporal` and optional `link_type_label` resolved via `CatalogSnapshot.linkTypeByName` (catalog miss -> `is_temporal: false`, `link_type_label` OMITTED); `is_in_effect` / `status` / `flags` OMITTED on the directed path (view-derived; a follow-up `traverse` surfaces them if needed). Wire emission (step 3), defensive `try/catch` WARN (step 4), non-persistence (step 5), non-replay (step 6) — ALL unchanged; only the trigger set (step 1) and the normalization dispatch table (step 2) grow. (e) **BR-43 v2.11** — the "Sequence inside the agentic loop" step no longer asserts "NO `graph_delta` emission"; the route synthesises the frame immediately after the `ingest_directed` `tool_result` per BR-41 v2.11. All other BR-43 steps (1..6 payload / dispatch / envelope / errors / audit / no-fire-and-forget) UNCHANGED. (f) §12 Known Technical Constraints: the existing bullet on catalog-snapshot dependency extended in-place to name `ingest_directed`; the existing bullet on non-persistence/non-replay extended in-place; NEW bullet documenting that partial-rejection `ingest_directed` emits a `graph_delta` covering ONLY the accepted subset (dropped items surface in the text channel via `report[i].status`, not in the graph — the graph never displays what does not exist in the persisted state); NEW bullet documenting the envelope-shape coupling with `ingestion` (three fields read; treated as `unknown` at value-time; type-only cross-module import permitted). (g) §13 Out of Scope: NEW bullet documenting the REVOKE of the v2.1 invariant "no `graph_delta` for ingestion tools"; the four `propose_*` handlers, `ingest_document`, and `get_ingestion_status` STAY out of the trigger set because they are not reachable from chat under v2.8+. Backwards-compatibility: clients that already dedupe by node `id` transparently accept the new frames; clients that switch on `source_tool` and enumerate the four legacy literals silently drop the new frame (safe fallback — same UX as v2.10 pre-fix; SPA is expected to add the fifth literal). §7 External Integrations: NO change (no new value/type imports beyond the type-only optional `IngestDirectedResult` from `ingestion/mcp/mcp-schemas.ts`). §9 Observability: existing WARN log shape `chat.graph_delta_normalize_failure` uniformly covers the fifth arm (no new log shape needed — normalizer signature is uniform); existing counter `chat_ingest_directed_total{ok}` unaffected (graph_delta emission is orthogonal to the ingest counter — it does NOT bump it). §10 Error Catalog: NO change (no new error code; a projector exception is absorbed at the WARN log level, not surfaced to the user). §11 Performance Budgets: NO change (the fifth arm is a pure in-memory map + a small lookup against a hot boot-time snapshot — dominated by the existing `< 50 ms p95` bound established in v2.1). Naturally gated by `CHAT_INGEST_ENABLED` (BR-44 v2.8): when the flag is `false` the chat catalog does not carry `ingest_directed` and the fifth trigger literal is UNREACHABLE at runtime. NO migration. NO schema change. NO new HTTP endpoint. NO new SSE frame (the same `graph_delta` frame gets a fifth `source_tool` literal — additive-only). NO new env var. NO new error code. NO code seam moves outside `service/graph-normalizer.ts` (dispatcher gains an arm) + `routes/conversations.routes.ts` (emission gate admits the fifth name). PRESERVED from v2.10.1: P2.1 canonical error-code mapping (§10 / §7 / §9 / BR-06 / BR-43); from v2.9: temporal & memory fidelity (BR-31 v2.9 / BR-33 v2.9 / BR-46 / BR-47); from v2.8: directed-ingestion catalog (BR-05 / BR-43 v2.8 steps 1..5 / BR-44) — only BR-43 v2.8's "Sequence inside the agentic loop" step is amended to reference the new BR-41 emission; from v2.7: graph-view snapshot v2 (BR-42); from v2.6: per-tool JSON Schema descriptors (BR-06 step 7); from v2.5: ontology-aware prompt (BR-18 v4); from v2.3: graph-view snapshot persistence; from v2.2: multi-row persistence (BR-29 / BR-31 / BR-32); from v2.1: `graph_delta` projection (BR-41 — trigger set widens; wire, defensive gates, non-persistence, non-replay all UNCHANGED). Reconciles with `chat.spec.md` v2.9.0 (BR-43 v2.6 step 9 — graph projection coupling pointer) + `openapi.yaml` v2.9.0 (`GraphDeltaEvent.source_tool` fifth-literal extension + new `ingest_directed` per-tool projection rule in the "graph_delta semantics" section). | sdd_render-graph-after-ingest_chat_spec-back |
