...
| Auth | Supabase Auth JWT validated by a Fastify `preHandler` middleware on every route under `/api/v1/search` and `/api/v1/provenance/*`. Single-owner -- no `User` entity, no role check (BR-16 of `.spec.md`, A20/A29). The Supabase service key never leaves the BFF (CLAUDE.md "Security"). PostgreSQL RLS is disabled. | CLAUDE.md default |
...
**Where to validate:** middleware -- the same `requireSupabaseJwt` Fastify `preHandler` used by `knowledge-graph` is mounted on `/api/v1/search` and `/api/v1/provenance/*`. JWKS cached for 10 min; verification happens **before** any DB access (BR-16 of `.spec.md`, A29, cenario C16).
