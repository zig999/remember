...
| MCP server | Same BFF process. This domain registers `search`, `get_provenance_link`, `get_provenance_attribute`, `get_provenance_fragment` on the **read-only MCP query transport** owned by `knowledge-graph` (`POST /api/v1/mcp/query` -- `knowledge-graph.back.md` BR-23). The two registrars (this domain + `knowledge-graph`) are composed at boot into one tool registry exposed by that single endpoint. Mirrored 1:1 in REST (`searchKnowledge`, `getProvenanceByLink` / `getProvenanceByAttribute` / `getProvenanceByFragment`); tool input Zod schemas are the same exports as the REST DTO Zod schemas. The pre-existing **ingest** transport `POST /api/v1/mcp` (owned by `ingestion`) is NOT used and NOT modified by this domain. **In-process consumer (v1.4.0):** the `chat` domain (v2.3+) consumes the same 4 tools as **in-process** agentic tools via `mcp.getTool('query', name)` against the SAME `McpServer` instance hosted on `POST /api/v1/mcp/query` (`chat.spec.md` BR-05 / `chat.back.md` BR-05). Chat's tool dispatcher opens its OWN `withReadOnly(pool, ...)` short transaction per invocation (same helper as the REST path), so the per-tool transaction policy already documented (Transaction policy row below) is unchanged by this consumer. The chat v2.3 async-ingestion tools (`start_async_ingestion`, `get_ingestion_status`) resolve on the `ingest` toolset (`mcp.getTool('ingest', name)`), **not** on `query` -- the catalog isolation between `query` and `ingest` toolsets is the invariant that keeps this domain read-only even with the chat write-bearing capability live. | New (this domain). |
...
> **P2.1 canonical taxonomy (v1.6.0).** Every `error.code` emitted by this mapper conforms to the project-wide NAMESPACED taxonomy adopted by amendment P2.1 (`docs/specs/_global/error-codes.md` "Canonical Taxonomy (P2.1 -- 2026-07-02)"). The seven §14 short codes (`STRUCTURAL_INVALID`, `UNKNOWN_TYPE`, `RULE_VIOLATION`, `TEMPORAL_INCOHERENT`, `DATE_UNJUSTIFIED`, `NOT_FOUND`, `INTERNAL`) are DEPRECATED and MUST NOT appear on either transport of this domain (they never did -- this domain has always emitted namespaced codes; P2.1 formalises the vocabulary). The mapper is the single point of truth that keeps the byte-identity invariant: given the same thrown business error, the REST 4xx body and the MCP `isError: true` payload carry the SAME `error.code` string byte-for-byte, the SAME `error.message` byte-for-byte, and the SAME `error.details` shape. Any future PR that introduces a per-transport code discriminator (a `code/mcpCode` pair, a transport-branching switch) MUST be rejected at code review -- P2.1 explicitly retires that construct.
...
**Description:** Parity is the single guard against drift between REST and MCP. Failures are blocking in CI (CLAUDE.md "Rule 9 -- Tests Verify Intent"). **P2.1 canonical taxonomy (v1.6.0).** The forced-error branch of the parity assertion MUST assert that the surfaced `error.code` is one of the NAMESPACED prefixes (`AUTH_*` / `VALIDATION_*` / `RESOURCE_*` / `BUSINESS_*` / `SYSTEM_*`) and is byte-for-byte identical across the two transports; a test that ever observes one of the seven deprecated §14 short codes (`STRUCTURAL_INVALID`, `UNKNOWN_TYPE`, `RULE_VIOLATION`, `TEMPORAL_INCOHERENT`, `DATE_UNJUSTIFIED`, `NOT_FOUND`, `INTERNAL`) MUST fail. This makes BR-25 the CI guard for this domain's compliance with the canonical registry -- parallel guards for the other MCP-bearing domains are declared in `compliance-audit.back.md` BR-14, `curation.back.md` BR-32 and `knowledge-graph.back.md` TC-04.
...
### BR-27 -- `listAcceptedFragments` is a read-only join with mandatory source filter, dedup, and tombstone exclusion
**Related UC:** UC-10
**Where to validate:** routes / service / repository -- the new Fastify handler `GET /api/v1/fragments/accepted` (operationId `listAcceptedFragments`, see `openapi.yaml` v1.3.0 path) is wired in `backend/src/modules/query-retrieval/routes/list-accepted-fragments.routes.ts` and dispatches to a new repository function `listAcceptedFragmentsBySource(filters, page)` in `backend/src/modules/query-retrieval/repository/`.

Layered enforcement:

1. **Routes (Zod) -- input precondition (mirrors `.spec.md` BR-20).** A Zod `z.object({ llm_run_id: z.string().uuid().optional(), raw_information_id: z.string().uuid().optional(), limit: z.coerce.number().int().min(1).max(100).default(20), offset: z.coerce.number().int().min(0).default(0) }).refine(v => v.llm_run_id !== undefined || v.raw_information_id !== undefined, { ... })` runs in the route `preHandler`. Both filters absent -> 422 `VALIDATION_INVALID_FORMAT` with `details = { requires_one_of: ["llm_run_id", "raw_information_id"] }`. A supplied filter that is not a syntactically valid UUID -> 422 `VALIDATION_INVALID_FORMAT` with `details.field` naming the offender. `limit` outside `[1, 100]` or `offset` < 0 -> 422 `VALIDATION_OUT_OF_RANGE`. No DB statement is issued on a 422 path. Pagination defaults mirror `searchKnowledge` (BR-21 of this back spec; `.spec.md` BR-18).

2. **Auth.** The same `requireNeonAuth` Fastify `preHandler` of BR-01 of this back spec is mounted on `/api/v1/fragments/accepted` -- single-owner JWT validation runs before the Zod refine. The endpoint is REST-only and is NOT registered on the MCP `query` toolset of BR-23 (curator helper; the LLM reaches accepted fragments through `search`).

3. **Repository (SQL).** One parameterized statement, no string interpolation of user input (CLAUDE.md "Security / Forbidden patterns"); wrapped in `withReadOnly(pool, ...)` so the listing + `count(*)` share one Postgres `READ ONLY` snapshot (consistent with the Transaction policy row of §1 and BR-23). The statement shape (faithful to `openapi.yaml` §1238-1376 and `.spec.md` UC-10 step 5):
   ...
   WITH page AS (
     SELECT f.id            AS fragment_id,
            ...
            f.llm_run_id    AS llm_run_id,
            f.created_at    AS created_at,
            r.id            AS raw_information_id,
            min(rc.chunk_index)              AS chunk_index,
   ...
   SELECT *, (SELECT count(*) FROM page) AS total
     FROM page
    ORDER BY received_at DESC NULLS LAST, created_at DESC, fragment_id ASC
    LIMIT $3 OFFSET $4;
   ```

   The bound parameters are `$1 = llm_run_id ?? null`, `$2 = raw_information_id ?? null`, `$3 = limit`, `$4 = offset` -- nullable inputs collapse the optional `WHERE` clause cleanly without dynamic SQL. `total` is computed BEFORE the `LIMIT` slice (window-subquery pattern; equivalent to `count(*) OVER ()` but avoids the per-row overhead at this listing's small page size). On `offset > 0` an empty `items` slice still carries the correct `total` (the SPA picker uses it for the page indicator).

4. **Deduplication.** `GROUP BY f.id, r.id` reduces a fragment that maps to multiple `raw_chunk` rows via `fragment_source` to exactly one row per `(fragment_id, raw_information_id)` pair; `min(rc.chunk_index)` selects the LOWEST supporting `chunk_index` for the surfaced `source.chunk_index` (deterministic; the full chunk chain remains reachable via `getProvenanceByFragment`, BR-18 of this back spec). The grouping is by `(f.id, r.id)` rather than by `f.id` alone because the schema does not forbid a single fragment from drawing chunks from multiple raws (cross-document corroboration via `propose_fragment` with `chunk_ids: uuid[]`); in practice the intersection filter or the `raw_information_id` filter collapses this to one row per fragment, but the join shape is correct under either filter combination.

5. **Tombstone exclusion (silent, per `.spec.md` BR-14).** The `NOT EXISTS (SELECT 1 FROM compliance_deletion cd WHERE cd.raw_information_id = r.id)` predicate is the structural mechanism. Consistent with BR-17 of this back spec (which is the 410-on-walk variant for provenance reads) -- here the SAME EXISTS predicate is inverted from "raise 410 when a tombstone is reached" to "silently drop the row" because the listing contract is "what is available to cite". The picker UI does NOT learn that the document was tombstoned; the row just doesn't appear (UC-10 alternative flow `5c`; `.spec.md` BR-14 explicit "for `listAcceptedFragments` (UC-10) the same principle is applied as a SILENT EXCLUSION rather than a 410"). The `compliance_deletion.raw_information_id` index (`compliance_deletion_raw_idx`, §2 Indexes table) is already in place; no new index is needed.

6. **Ordering.** `ORDER BY r.received_at DESC NULLS LAST, f.created_at DESC, f.id ASC` -- newest documents first (the curator usually points at the latest errata), then newest fragments inside the same document, then a stable UUID tiebreaker so two identical requests return rows in the same order (same defensive pattern as BR-15 of this back spec). `NULLS LAST` is a defensive guard -- `raw_information.received_at` is `NOT NULL DEFAULT now()`, but the ordering survives a future relaxation of that constraint without flipping page order.

7. **Pagination defaults align with `searchKnowledge`.** `limit = 20` default / `100` max, `offset = 0` default (BR-21 of this back spec; `.spec.md` BR-18). `total` is the `count(*)` from the CTE BEFORE the `LIMIT` slice. The SPA picker is a small dropdown; the typical call returns `total <= 100` and never paginates past the first page in practice -- but the contract supports it.

8. **Service-level response shape.** The repository row tuples are folded into `AcceptedFragmentItem` (one item per row) with `source = { raw_information_id, chunk_index, source_type, received_at, document_title }`; `total`, `limit`, `offset`, `items[]` become `AcceptedFragmentList`. The Fastify handler sends `reply.status(200).send({ ok: true, result: AcceptedFragmentList })` per BR-26 of this back spec -- the success envelope rule extends to `listAcceptedFragments` mechanically (the openapi `AcceptedFragmentListEnvelope` of `openapi.yaml` lines 1362-1368 cites "back-spec BR-26" for this; the back-spec wrap rule itself is unchanged).

9. **Logging (pino).** The handler emits the `request_id`, `route = 'listAcceptedFragments'`, `actor = 'owner'`, `outcome`, `latency_ms` fields documented in the Logging row of §1, plus listing-specific fields: `filter_llm_run_id` (UUID or `null`), `filter_raw_information_id` (UUID or `null`), `intersection` (boolean -- `true` when BOTH filters supplied), `result_count` (rows returned in this page), `total` (count before pagination), `tombstone_excluded_count` (count of `fragment_id` values that would have matched but were dropped by the `NOT EXISTS compliance_deletion` predicate -- computed via a second cheap CTE-style subquery when the metric is enabled by env, or zero by default; this feeds the compliance feedback loop already documented in §1 Observability). The `text` column of accepted fragments is NEVER logged (CLAUDE.md "Security / Forbidden patterns: Logging sensitive fields"). Query elapsed time is recorded via `process.hrtime.bigint()` around the `withReadOnly` call and reported as `latency_ms` -- same instrumentation as the other four routes.

10. **Empty result is HTTP 200.** Filters matching no accepted fragment (no row, or every match tombstoned) -> `200` with `result = { total: 0, items: [], limit, offset }`. The endpoint does NOT validate referential existence of `llm_run_id` / `raw_information_id` (UC-10 alternative `5b`) -- it is a listing, not a point read; the SPA picker only ever calls it with ids it just received from a prior write.

11. **No new error codes; no DDL; no MCP surface.** All error codes referenced (`AUTH_*`, `VALIDATION_INVALID_FORMAT`, `VALIDATION_OUT_OF_RANGE`, `SYSTEM_INTERNAL_ERROR`, `SYSTEM_SERVICE_UNAVAILABLE`) are already registered in the global catalog and reused from BR-01 / BR-02 / BR-04 / BR-20 of this back spec. The endpoint is REST-only -- it is NOT registered on the MCP `query` toolset (BR-23 enumerates the four MCP tools this domain contributes; `listAcceptedFragments` is intentionally NOT among them). The new statement adds no new index, no new column, no new table, and no new error code; the existing `information_fragment` partial GIN (`status = 'accepted'`), the `fragment_source` PK btree, the `raw_chunk.raw_information_id` FK index, and the `compliance_deletion.raw_information_id` index cover the join and the tombstone predicate (see §2 Indexes table).

**Description:** A new READ-ONLY listing endpoint over `information_fragment WHERE status = 'accepted'`, joined down to `raw_information` for the source-context label, scoped by at least one of `llm_run_id` / `raw_information_id`, deduplicated per fragment (`min(chunk_index)` for the surfaced source ref), tombstone-excluded silently, ordered `received_at DESC NULLS LAST, created_at DESC, id ASC`, paginated offset/limit (defaults 20 / 0; max 100), wrapped in the standard REST envelope (BR-26). Consumed by the SPA `CorrectionForm` DateJustification picker (`curation.back.md` BR-15; ADR A14 -- "the system NEVER invents dates").
**Error returned:** HTTP 401 -- `AUTH_UNAUTHORIZED` / `AUTH_TOKEN_INVALID` / `AUTH_TOKEN_EXPIRED` (reuse BR-01); HTTP 422 -- `VALIDATION_INVALID_FORMAT` (missing filter precondition or non-UUID input) / `VALIDATION_OUT_OF_RANGE` (limit/offset); HTTP 500 -- `SYSTEM_INTERNAL_ERROR` (reuse BR-20); HTTP 503 -- `SYSTEM_SERVICE_UNAVAILABLE` (reuse BR-20). No new error code is added by this BR.
...
| `chat` domain agentic loop (in-process consumer, v1.4.0) | In-process function call via `mcp.getTool('query', name)` against the same `McpServer` instance hosted on `POST /api/v1/mcp/query` (chat does NOT cross the HTTP boundary to consume the tools -- `chat.back.md` BR-05). | Dispatch any of the 4 read tools (`search`, `get_provenance_link`, `get_provenance_attribute`, `get_provenance_fragment`) inside the Anthropic agentic tool-use loop of `POST /api/v1/conversations/:id/messages`. | Per-tool-call: the same `withReadOnly` short transaction policy applies; the chat-side ceiling is governed by `chat.back.md` (Anthropic streaming + per-iteration deadlines) and inherits the `pg` statement timeout. | None at this layer -- the chat domain's own loop owns the abort surface (socket close, SSE error frame); a slow read tool surfaces to the model as a delayed `tool_result` and is bounded by the parent transaction timeout. The chat v2.3 async-ingestion tools (`start_async_ingestion`, `get_ingestion_status`) resolve on the `ingest` toolset and do NOT touch this domain (`chat.back.md` BR-43 / BR-45). |
...
- **Chat is an in-process consumer of the 4 read tools (v1.4.0).** The `chat` domain (v2.3+) resolves `search`, `get_provenance_link`, `get_provenance_attribute`, `get_provenance_fragment` via `mcp.getTool('query', name)` and dispatches them inside its Anthropic agentic loop (`chat.spec.md` BR-05 / `chat.back.md` BR-05). Chat opens its OWN `withReadOnly(pool, ...)` transaction per tool call -- the same helper used by the REST routes and by the HTTP MCP dispatch (BR-23). The service layer of this domain is therefore re-entered by THREE call paths (REST, HTTP MCP, in-process MCP via chat) on every tool name in this domain's catalog; all three share one core. Adding a new tool to this domain auto-exposes it to chat once it is registered on the `McpServer` instance -- there is no chat-side allow-list.
- **Catalog isolation between `query` and `ingest` toolsets is load-bearing (v1.4.0).** The chat v2.3 async-ingestion capability (`start_async_ingestion` + `get_ingestion_status`, gated by `CHAT_INGEST_ENABLED`) is resolved on the `ingest` toolset (`mcp.getTool('ingest', name)`), **not** on `query`. The two toolsets share the same `McpServer` instance at the registration plane but are NAMESPACE-distinct at the lookup plane -- a tool registered on `query` cannot be invoked through the `ingest` lookup path, and vice versa. This invariant is what keeps this domain read-only even when chat carries a write-bearing tool: the chat tool dispatcher resolves each name in its catalog against the correct toolset namespace, so any future `query` tool MUST stay read-only (CLAUDE.md "Anti-patterns / Data") regardless of whether chat is later configured to also import from `query` for write-shape tools (rejected by definition; would require an OpenAPI + spec change and a fresh BR).
- **Audit-row policy is unchanged by the chat consumer.** Chat persists its OWN audit rows (`chat_tool_call` per `chat.back.md` BR-32) for every tool dispatch -- these rows live in the chat schema and are scoped to a conversation, not to a `query-retrieval` run. This domain still writes NO audit row of its own on tool dispatch (BR-23 "MCP tool calls do NOT write `tool_call` audit rows"); the chat-side audit is at a different layer (conversation turn, not service call) and does not interact with this domain's persistence model.
---
...
- **Chat v2.3 async-ingestion tools (`start_async_ingestion` / `get_ingestion_status`).** Resolved on the `ingest` toolset, NOT on `query` (`chat.back.md` BR-05 / BR-43 / BR-45). This domain registers ONLY the 4 read tools listed in BR-23; the ingestion-side tools are owned by `ingestion.back.md` BR-30 / BR-31 and surfaced to chat through the namespace-distinct `ingest` toolset.
- **Write-bearing tools.** Any future PR adding a tool to this domain's `registerQueryRetrievalToolset` that opens a write transaction or touches a non-read-only helper MUST be rejected at code review. The read-only invariant is enforced by the per-dispatch `BEGIN READ ONLY` of `withReadOnly` -- a write attempt fails with a Postgres `read_only_sql_transaction` error and surfaces as `SYSTEM_INTERNAL_ERROR` (BR-20). The chat consumer relies on this invariant (it opens its own `BEGIN READ ONLY` per `chat.back.md` BR-05); breaking it would silently surface a write capability to the agentic loop.
- **Chat-owned audit rows (`chat_tool_call`).** Owned by `chat` (`chat.back.md` BR-32) -- outside the lifecycle of this domain. This domain does not read or write `chat_tool_call`; the chat side persists arguments and results in the conversation context, while this domain's only audit-equivalent is the structured pino log line emitted by the Logging row of the Stack table.
---
...
| 1.4.0 | 2026-06-22 | Back Spec Agent | minor (additive) | **Chat downstream-consumer reverse declaration (no implementation change).** Mirrors `query-retrieval.spec.md` v1.3.0 (which added `chat` to §7 Cross-Domain Dependencies) into the back-spec surface, without touching the implementation, the OpenAPI, the schema, the BR catalog, the state machines, the events, the error codes, the migrations or the data model. The chat domain (v2.3+) resolves the 4 read tools of this domain (`search`, `get_provenance_link`, `get_provenance_attribute`, `get_provenance_fragment`) via `mcp.getTool('query', name)` against the SAME `McpServer` instance hosted on `POST /api/v1/mcp/query` (registered by BR-23); chat opens its own `withReadOnly(pool, ...)` per tool dispatch (the same helper used by the REST path and by the HTTP MCP dispatch), so the per-tool transaction policy already documented in BR-23 + Transaction policy is unchanged by the new consumer. The chat v2.3 async-ingestion capability (`start_async_ingestion`, `get_ingestion_status`, gated by env `CHAT_INGEST_ENABLED`) resolves on the `ingest` toolset (NOT on `query`) -- the catalog-isolation invariant between `query` and `ingest` toolsets is what keeps this domain read-only even with the chat write-bearing capability live. Updated: §1 Stack table (MCP server row appended an explicit in-process chat-consumer note + the catalog-isolation invariant); §6 External Integrations (new row '`chat` domain agentic loop (in-process consumer, v1.4.0)' listing chat as an in-process consumer of the 4 read tools, with the same per-tool transaction ceiling as REST / HTTP MCP); §7 Known Technical Constraints (3 new bullets: chat as in-process consumer; `query` vs `ingest` namespace isolation; audit-row policy unchanged); §8 Out of Scope (3 new bullets: chat v2.3 ingestion tools resolved on `ingest`; write-bearing tools in this domain forbidden; `chat_tool_call` audit owned by chat). No new BR, no new schema, no new error code, no migration, no DDL, no new external integration in the traditional sense (the chat consumer is in-process, not network-bound); the openapi.yaml is UNCHANGED (still v1.2.0). Coordinated with `chat.spec.md` v2.3 §7 reverse-declaration requirement and with `query-retrieval.spec.md` v1.3.0 (mirror, additive). | sdd_query-retrieval_spec-back |
| 1.5.0 | 2026-06-24 | Back Spec Agent | minor (additive) | **`listAcceptedFragments` repository + route rule (new BR-27).** Adds BR-27 to §3 specifying the implementation contract for the new `GET /api/v1/fragments/accepted` endpoint (operationId `listAcceptedFragments`) defined by `openapi.yaml` v1.3.0 and `query-retrieval.spec.md` v1.4.0 (UC-10 / BR-20). Read-only join `information_fragment -> fragment_source -> raw_chunk -> raw_information` filtered by `f.status = 'accepted'` and by at least one of `llm_run_id` / `raw_information_id` (precondition enforced by Zod refine in the route `preHandler` before any DB call; missing both -> 422 `VALIDATION_INVALID_FORMAT` with `details.requires_one_of`); intersection when both supplied. Per-fragment dedup via `GROUP BY f.id, r.id` with `min(rc.chunk_index)` surfaced as `source.chunk_index` (deterministic; remaining anchors reachable via `getProvenanceByFragment`). Silent tombstone exclusion via `NOT EXISTS (SELECT 1 FROM compliance_deletion cd WHERE cd.raw_information_id = r.id)` -- mirrors the 410-on-walk EXISTS predicate of BR-17 but inverted to a silent drop per `.spec.md` BR-14 (listing contract is 'what is available to cite'; the picker UI does not learn about the tombstone). Ordering `r.received_at DESC NULLS LAST, f.created_at DESC, f.id ASC` (stable tiebreaker, same defensive pattern as BR-15). `withReadOnly` wrapper -- one short `READ ONLY` transaction shares the snapshot between the listing CTE and the `count(*)` total. Pagination defaults mirror `searchKnowledge` (BR-21): `limit` 20 default / 100 max, `offset` 0 default; `total` computed BEFORE the slice. Success body wrapped as `{ ok: true, result: AcceptedFragmentList }` per BR-26 (the success envelope rule extends to this endpoint mechanically). Pino logging adds `filter_llm_run_id` / `filter_raw_information_id` / `intersection` / `result_count` / `total` / `tombstone_excluded_count` and the request `latency_ms`; the fragment `text` is NEVER logged (CLAUDE.md Security). REST-only -- explicitly NOT registered on the MCP `query` toolset of BR-23 (curator helper; the LLM reaches accepted fragments through `search`). No new error code (reuses `AUTH_*` / `VALIDATION_INVALID_FORMAT` / `VALIDATION_OUT_OF_RANGE` / `SYSTEM_INTERNAL_ERROR` / `SYSTEM_SERVICE_UNAVAILABLE`); no migration; no new index (existing partial GIN on `information_fragment.status = 'accepted'`, `fragment_source` PK btree, `raw_chunk.raw_information_id` FK index, and `compliance_deletion_raw_idx` cover the join + tombstone predicate per §2 Indexes); no change to data model, state machines, events, or external integrations. Coordinated with `query-retrieval.spec.md` v1.4.0 (UC-10 + BR-20) and `openapi.yaml` v1.3.0 (path + schemas `AcceptedFragmentListEnvelope` / `AcceptedFragmentList` / `AcceptedFragmentItem` / `AcceptedFragmentSourceRef`). | sdd_improve_6_spec-back |
| 1.6.0 | 2026-07-02 | Back Spec Agent | patch (documentation) | **P2.1 canonical taxonomy alignment (documentation-only).** Anchors this back-spec into the project-wide canonical error-code taxonomy adopted by amendment P2.1 (`docs/specs/_global/error-codes.md` "Canonical Taxonomy (P2.1 -- 2026-07-02)"). BR-24 extended with an explicit invariant paragraph: the mapper emits ONLY namespaced codes; the seven §14 short codes are DEPRECATED and MUST NOT appear on either transport; the byte-identity contract (same `error.code` / `error.message` / `error.details` shape across REST and MCP for the same thrown business error) is the single point of truth against a per-transport `code/mcpCode` pair (P2.1 retires that construct). BR-25 extended with an explicit parity assertion clause: the forced-error branch MUST assert the surfaced code is a NAMESPACED prefix (`AUTH_*` / `VALIDATION_*` / `RESOURCE_*` / `BUSINESS_*` / `SYSTEM_*`) AND is byte-identical across transports; observing any of the seven deprecated short codes MUST fail the test. This makes BR-25 the CI guard for P2.1 compliance in this domain; parallel guards in `compliance-audit.back.md` BR-14, `curation.back.md` BR-32, `knowledge-graph.back.md` TC-04. NO new BR, NO new test file, NO change to the mapper, NO new error code, NO new HTTP status, NO schema change, NO migration -- this domain has ALWAYS emitted only namespaced codes; P2.1 formalises the vocabulary and this revision cross-references the canonical registry from the back-spec. Coordinated with `query-retrieval.spec.md` v1.5.0 (mirror documentation-only note in §6 Error Behaviors) and `openapi.yaml` v1.5.0 (mirror note in `info.description`). | sdd_query-retrieval_spec-writer |
