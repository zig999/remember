...
> Version: 1.5.1 | Status: draft | Layer: permanent
...
**Alternative flows:**
- `1a` No ambient `llm_run_id` (run not found, or run not `running`) -> error envelope `{ ok: false, error.code: <namespaced code> }`; no DB write. REST mirror returns 409 with `BUSINESS_RUN_NOT_RUNNING` when the run exists but is not `running`, and 404 with `RESOURCE_NOT_FOUND` when the run id does not exist. Under P2.1 the MCP transport surfaces the **same** codes byte-identically on its wire (`BUSINESS_RUN_NOT_RUNNING` for the not-running case; `RESOURCE_NOT_FOUND` for the unknown-id case) — the previous "MCP collapses both into `STRUCTURAL_INVALID`" behaviour is retired; the propose-* REST mirrors distinguish the two via HTTP status while the MCP wire distinguishes via the error.code alone.
- `2a` `text` > 1000 chars / `confidence` out of [0,1] / `chunk_ids` empty -> error envelope `{ ok: false, error.code: "VALIDATION_OUT_OF_RANGE" }` (or `VALIDATION_REQUIRED_FIELD` when the field is absent, `VALIDATION_INVALID_FORMAT` when the type/format is wrong — Zod-discriminated per P2.1); the `ToolCall` is recorded with `validation_outcome = 'rejected'`.
- `2b` Some `chunk_id` does not exist -> error envelope `{ ok: false, error.code: "RESOURCE_NOT_FOUND" }` (P2.1 namespaced); `ToolCall` recorded with `validation_outcome = 'rejected'`.
- `2c` Some `chunk_id` belongs to a different `RawInformation` than the run's input -> error envelope `{ ok: false, error.code: "VALIDATION_INVALID_FORMAT", message: "chunk is not part of this run's source" }` (P2.1 namespaced — cross-table compatibility mismatch); `ToolCall` recorded with `validation_outcome = 'rejected'`.
...
**Pre:** Caller holds a valid JWT and the ambient `llm_run_id` (`running`). `node_type` is one of the 9 catalog `NodeType.name` values (the 8 of seed §15.1 — `Person`, `Organization`, `Project`, `Event`, `Role`, `Category`, `Concept`, `Location` — plus `Document` via migration `0002`); `name` is non-empty.
...
2. Service validates structurally and looks up `NodeType` by name (`BUSINESS_UNKNOWN_NODE_TYPE` on miss — P2.1 namespaced).
...
- `2a` `node_type` not in the seeded catalog -> `BUSINESS_UNKNOWN_NODE_TYPE` (P2.1 namespaced); `ToolCall` `validation_outcome = 'rejected'`.
- `2b` `name` blank or > 500 chars -> `VALIDATION_REQUIRED_FIELD` (blank) or `VALIDATION_OUT_OF_RANGE` (>500) — P2.1 namespaced; `ToolCall` `validation_outcome = 'rejected'`.
...
- `2a` Unknown `link_type` -> `BUSINESS_UNKNOWN_LINK_TYPE` (P2.1 namespaced); `ToolCall.validation_outcome = 'rejected'`.
- `2b` `(source_node_type, link_type, target_node_type)` not authorised by any vigent `LinkTypeRule` -> `BUSINESS_LINK_RULE_VIOLATION` (P2.1 namespaced); `ToolCall.validation_outcome = 'rejected'`.
- `2c` `valid_from` given without `valid_from_basis`, or `valid_from_basis = 'stated'` without a fragment containing the date in the text, or `valid_from ≥ valid_to` -> `BUSINESS_TEMPORAL_INCOHERENT` (P2.1 namespaced); `ToolCall.validation_outcome = 'rejected'`.
- `2d` `requires_valid_from = true` and `valid_from` not derivable from any of the three bases -> `BUSINESS_DATE_UNJUSTIFIED` (P2.1 namespaced); `ToolCall.validation_outcome = 'rejected'`.
...
- `2f` Any `fragment_id` not real or not in this run's source -> `RESOURCE_NOT_FOUND` when the fragment id resolves to no row, or `VALIDATION_INVALID_FORMAT` when the fragment exists but is anchored to a different run's source (P2.1 namespaced — cross-table compatibility mismatch); `ToolCall.validation_outcome = 'rejected'`.
- `3a` Re-affirmation of a vigent link — same `(source_node_id, link_type_id, target_node_id)` already vigent AND `change_hint = 'none'` AND no textual succession/correction signal in the cited fragments -> consolidation: no new row; existing `Provenance` accumulates the new fragments (§18). `valid_from` equality is **not** required when `link_type.allows_multiple_current = true` (the partial dup-guard is scoped by `(source, link_type, target)` — `valid_from` is deliberately not in the key); a `received`-fallback divergence between documents (FR-001) still consolidates. For functional link types (`allows_multiple_current = false`) with the same target, consolidation applies regardless of `valid_from` equality as well — the divergent date is informational, because succession (alt `3b`) requires a **different** target plus a textual signal. The vigent row's `valid_from` / `valid_from_basis` are preserved as-is. `ToolCall.validation_outcome = 'consolidated'`.
...
**Actor:** LLM (inside an active `LLMRun`, via MCP or via UC-12) OR Owner (via REST mirror).
**Pre:** Same as UC-10 plus: `key` is one of the 16 catalog `AttributeKey.key` values (10 of seed §15.3 + 6 via `0002`: `Document.doc_type`, `Location.city`/`address`, `Concept.definition`, `Event.end_date`/`event_type`) scoped by the node's `node_type`; `value` is parseable as the key's `value_type` (date / number / text / bool, §3.4, §13.1).
...
- `2g` `value` does not parse as `key.value_type` (e.g., key is `date` but `value = "tomorrow"`) -> `VALIDATION_INVALID_FORMAT` (P2.1 namespaced); `ToolCall.validation_outcome = 'rejected'`.
- `2h` `key.node_type_id` does not match the node's `node_type_id` -> `VALIDATION_INVALID_FORMAT` (P2.1 namespaced — cross-table compatibility mismatch); `ToolCall.validation_outcome = 'rejected'`.
...
3. Orchestrator reads `model`, `prompt_version` from the run, loads the prompt module matching `prompt_version` (e.g. `extraction.v1.ts`) and a CatalogSnapshot of the live catalog (§15 seed + migration `0002`: 9 NodeTypes / 13 LinkTypes + 28 rules / 16 AttributeKeys) into memory (BR-26).
...
> **MCP one-shot trigger — `ingest_document` (Emenda v7.4; `ingestion.back.md` BR-30).** An external MCP client (e.g. Claude Desktop) can perform UC-01 + UC-12 in a single call via the `ingest_document` tool of the `ingest` toolset: it creates the `RawInformation` + chunks + `running` `LLMRun` (UC-01) and then runs THIS use case (UC-12) server-side, returning the run summary. If the content was already ingested it returns `outcome = "already_ingested"` without re-extracting (idempotency, BR-08). The extraction LLM is the server's (`ANTHROPIC_API_KEY`); the same synchronous, connection-held latency profile above applies. MCP-facing only — the REST surface keeps UC-01 (`POST /raw-information`) and UC-12 (`POST /llm-runs/:id/run`) as separate endpoints.

> **MCP directed-ingestion trigger — `ingest_directed` (`ingestion.back.md` BR-34).** Distinct sibling of `ingest_document`: same intake-then-write shape, but the caller supplies the structured payload (fragments / nodes / attributes / links) directly and the server runs **no LLM** on this path — the orchestrator dispatches the four `propose_*` handlers in dependency order and returns a synchronous per-item report. UC-14 below documents this composition. The two MCP tools coexist intentionally — `ingest_document` is the right answer for callers that have unstructured documents and want extraction; `ingest_directed` is the right answer for callers that already know what they want to ingest (typically the chat agentic loop, which structures the turn context client-side and cannot afford to block on a server-side LLM round-trip).

### UC-13 — RETIRED in v1.4.0 (was: Start an asynchronous ingestion — intake now, extract in background)

> Retired because its sole consumer (the chat agentic loop) now uses `ingest_directed` (UC-14), which is deterministic, synchronous and bounded — collapsing the previous "fire-and-forget + poll `get_ingestion_status`" pattern into a single call that returns the per-item report inline. The detached-promise lifecycle (and its orphan-on-restart hazard, formerly UC-13 alt 5a) is eliminated on the chat path.
>
> Traceability: see `ingestion.back.md` v1.4.0 / v1.4.1 (BR-32 marked WITHDRAWN; BR-34 documents the replacement) and the v1.4.0 changelog entry below. The UC number `13` is **reserved** — not reused — so historical references (back-spec, prior changelogs, audit logs) remain unambiguous. `get_ingestion_status` (BR-31) remains in place for the non-chat polling channel after a client timeout on `ingest_document` (BR-30) — it is NOT retired alongside UC-13.

---

### UC-14 — Directed ingestion (deterministic composition; no server LLM)

**Actor:** External MCP client (typically the chat agentic loop, but any MCP-capable Owner-side client). Internally composes UC-01 (intake) and the four propose-* writers (UC-08..UC-11) in dependency order, with NO call to UC-12 — the directed orchestrator dispatches the propose-* service functions in-process and does **not** invoke the Anthropic SDK on this path.

**Pre:** Caller is authenticated (HTTP MCP transport: Neon Auth JWT verified by `requireNeonAuth`; local stdio MCP transport: local-process trust boundary per `knowledge-graph.back.md` BR-01 stdio carve-out). The payload Zod-parses cleanly as `IngestDirectedMcpInputSchema` (`ingestion.back.md` BR-34): `fragments[]` is non-empty; `nodes[]` is non-empty; every `*_ref` referenced by `attributes[]` / `links[]` exists in the local `ref -> item` maps of this call. `valid_from_basis` (when supplied on an attribute or link) is `'stated'` or `'document'` (the public `ValidFromBasis` enum — `'received'` is server-internal and rejected from callers, BR-16). **The `env.CHAT_INGEST_ENABLED` flag is `true`** when the caller is the chat module — when `false`, `ingest_directed` is not advertised in the chat agent's tool catalog (the tool is still registered on the `ingest` toolset itself; an external MCP client like `mcp-remote` or Claude Desktop can always call it directly, BR-34 "Chat integration" / "Rollout flag"). The `ANTHROPIC_API_KEY` is **irrelevant** on this path (no LLM is invoked).

**Post:** On a clean intake: a `RawInformation` row exists (synthesised content = the caller's `fragments[].text` concatenated with a trailing timestamp + nonce line, so `content_hash` is unique per call — there is **no `noop_existing` branch on this path**); the run's chunk(s) exist; an `LLMRun` row exists with `model = 'directed'`, `prompt_version = 'directed-v1'` (sentinels — the orchestrator never calls `selectPromptModule` / `runExtraction` for this run); the dispatched `propose_*` writes are persisted (one `tool_call` row per dispatched handler, BR-23); the run is closed `status = 'completed'` (the directed orchestrator never closes the run as `failed` — a per-item rejection is an item-level outcome, not a run-level fatal failure). The MCP response carries `{ outcome: 'ingested', raw_information_id, llm_run_id, chunk_count, run: { ..., affected_nodes }, report: ItemReport[], summary: { fragments, nodes, attributes, links, accepted, consolidated, superseded_previous, needs_review, uncertain, disputed, rejected, error, dependency_failed } }`. `result.run.affected_nodes` is populated synchronously (BR-33). On an intake failure before the run opens, no rows are written and the response carries the matching MCP envelope error code (see Alternative flows).

**Main flow:**
1. MCP client invokes tool `ingest_directed` with `{ fragments[], nodes[], attributes?[], links?[], source_label? }` (full schema in `ingestion.back.md` BR-34 "Tool schema").
2. BFF Zod-parses the input. Failure -> envelope `{ ok: false, error.code: <VALIDATION_REQUIRED_FIELD | VALIDATION_INVALID_FORMAT | VALIDATION_OUT_OF_RANGE> }` (P2.1 namespaced — Zod-discriminated); no run row is opened, no `tool_call` row is written (there is nothing to audit against yet).
3. BFF synthesises the raw content and opens the run in **one transaction** (BR-19): concatenates `fragments[].text` (one per line, prefixed with `[ref]`) plus a trailing `-- directed_at=ISO8601 nonce=base64url(16 bytes)` line — the nonce makes `content_hash` distinct per call. `ingestRawInformation` (UC-01 main flow) persists the `RawInformation` (`source_type = 'chat'`, `metadata.directed = true`, `metadata.source_label` when supplied), the chunk(s), and the `LLMRun` (`status = 'running'`, sentinels `model='directed'` / `prompt_version='directed-v1'`).
4. The directed orchestrator dispatches the items in the order `fragments -> nodes -> attributes -> links` — each dispatched `propose_*` call is its own transaction (BR-19); the orchestrator does NOT wrap the whole dispatch in a single transaction:
   - **fragments:** each `{ ref, text }` invokes `proposeFragmentHandler` with `confidence: 1.0` and `chunk_ids = [the run's chunk id(s) covering the fragment's line]`. On `ok: true` the orchestrator records `ref -> fragment_id`.
   - **nodes:** each `{ ref, node_type, name, node_id?, aliases? }` either (a) when `node_id` is supplied — a **pin** — looks up the active node by id (`RESOURCE_NOT_FOUND` when the id does not exist, `VALIDATION_INVALID_FORMAT` when the node exists but `status != 'active'` — P2.1 namespaced; the pin path skips entity resolution intentionally), or (b) invokes `proposeNodeHandler` (UC-09) and forwards the BR-24 outcome (`matched_existing` / `created_new` / `needs_review`) verbatim into the report. On `ok: true` the orchestrator records `ref -> node_id`.
   - **attributes:** each item resolves `node_ref` and `evidence_ref` against the in-memory maps; on success, invokes `proposeAttributeHandler` (UC-11) with `confidence: 1.0`, `fragment_ids: [evidence_id]`, and the caller's `valid_from` / `valid_from_basis` (default `'stated'`). The BR-25 outcome is forwarded verbatim.
   - **links:** mirror of attributes via `proposeLinkHandler` (UC-10).
5. **Cascade rule.** When a dispatched item's `*_ref` dependency has failed (the referenced fragment or node was rejected at its own step, or the pin lookup failed), the item is **skipped** with a synthetic per-item report entry `{ ref, kind, status: 'dependency_failed', reason: <first-missing-dependency-ref> }`. No `propose_*` call is issued for the cascaded item; **no `tool_call` row is written** for it (the handler did not run; BR-23 is a writer invariant, not a writer-or-cascade invariant). Cascade is per-item, not transitive (the cascaded item registers no ref to cascade from).
6. After every item has been dispatched-or-cascaded, the orchestrator closes the run via UC-07 with `status = 'completed'`. The terminal status is **always `completed`** on a successful intake (a `propose_*` rejection on a single item is an item-level outcome, not a run-level fatal failure; the BR-29 fatal-failure path is unreachable here because there is no Anthropic call that can fatally fail).
7. BFF returns the success envelope with the per-item report and the summary; the response is bounded by the dispatched-handler latency (no LLM round-trip), typically seconds at v1 scale.

**Alternative flows:**
- `1a` Caller's `env.CHAT_INGEST_ENABLED = false` and the caller is the chat agent -> `ingest_directed` is absent from the chat agent's tool catalog at boot; the chat model cannot emit it. Direct callers (external MCP clients via `POST /api/v1/mcp/ingest` or the local stdio transport) are **not** affected — the tool is still registered on the `ingest` toolset itself (BR-34 "Rollout flag").
- `2a` Zod parse failure on the input -> `{ ok: false, error.code: <VALIDATION_REQUIRED_FIELD | VALIDATION_INVALID_FORMAT | VALIDATION_OUT_OF_RANGE> }` (P2.1 namespaced — Zod-discriminated). No `RawInformation` is written; no `tool_call` row.
- `3a` Intake failure (UC-01 alt 5a — chunker error, DB outage, schema violation, etc.) -> the handler catches and returns `{ ok: false, error.code: "SYSTEM_SERVICE_UNAVAILABLE" }` when `isPgUnavailable(err)`, otherwise `{ ok: false, error.code: "SYSTEM_INTERNAL_ERROR" }` (P2.1 namespaced) with a generic message (raw `err.message` logged server-side only, never returned). No partial run exists. No `tool_call` row.
- `4a` A dispatched `propose_*` returns a rejection envelope (any of the P2.1 namespaced codes: `VALIDATION_REQUIRED_FIELD` / `VALIDATION_INVALID_FORMAT` / `VALIDATION_OUT_OF_RANGE` / `RESOURCE_NOT_FOUND` / `BUSINESS_UNKNOWN_NODE_TYPE` / `BUSINESS_UNKNOWN_LINK_TYPE` / `BUSINESS_UNKNOWN_ATTRIBUTE_KEY` / `BUSINESS_LINK_RULE_VIOLATION` / `BUSINESS_TEMPORAL_INCOHERENT` / `BUSINESS_DATE_UNJUSTIFIED`; or the confidence-floor business outcome `BELOW_CONFIDENCE_FLOOR` which is NOT an error.code — it lives in `result.outcome.reason`) -> the rejection is **forwarded verbatim** into `result.report[i]`; the top-level envelope stays `{ ok: true, ... }`; the run still closes `completed`; a `tool_call` row is still written for that handler invocation with `validation_outcome = 'rejected'` (BR-23).
- `4b` A dispatched `propose_*` returns a business outcome other than rejection (`matched_existing` / `created_new` / `needs_review` / `consolidated` / `superseded_previous` / `disputed` / `uncertain`) -> the outcome is forwarded verbatim into `result.report[i]`; counters in `result.summary` are incremented accordingly; the top-level envelope stays `{ ok: true, ... }`.
- `5a` A `*_ref` dependency has failed at dispatch time (the upstream item was rejected, or a node pin failed its active-node lookup) -> the item is cascaded with `{ ref, kind, status: 'dependency_failed', reason }`; `summary.dependency_failed` is incremented; no `tool_call` row; no `propose_*` call.
- `6a` Re-invocation of `ingest_directed` with the **same** structured payload -> a **new** `RawInformation` / `LLMRun` are created (the per-call nonce in step 3 makes `content_hash` unique), but the per-item `propose_*` calls hit the standard graph-consolidation path (BR-25): re-affirming a previously accepted link / attribute lands as `consolidated` (provenance accumulates on the existing row; no second vigent row is inserted). This preserves §18 "re-affirmation consolidates, never duplicates" at the graph layer while keeping every directed call uniquely audit-addressable at the run layer.

**Related endpoint / tool:** MCP tool `ingest_directed` on the `ingest` toolset (advertised by BOTH MCP transports — HTTP `POST /api/v1/mcp/ingest` and local stdio `backend/src/mcp-stdio.ts`). **No REST mirror** (same rationale as `ingest_document` / former `start_async_ingestion`: the REST surface already exposes UC-01 and the four propose-* mirrors as separate endpoints — a REST caller wanting directed-ingestion semantics calls them in sequence).

> **Distinction from UC-12 (`runLlmExtraction`), BR-30 (`ingest_document`) and the retired UC-13 (`start_async_ingestion`).**
> - **UC-12 `runLlmExtraction`**: REST + synchronous, operates over an existing `running` `LLMRun`; drives the server's Anthropic SDK tool-use loop. LLM-bound (minutes per document).
> - **BR-30 `ingest_document`**: MCP + synchronous, creates the run and drives extraction via the server's Anthropic. Same latency profile as UC-12.
> - **UC-13 `start_async_ingestion` (RETIRED, v1.4.0)**: previously MCP + asynchronous detached-promise (server Anthropic); now retired — the chat path is collapsed into UC-14.
> - **UC-14 `ingest_directed` (this UC)**: MCP + synchronous, creates the run and dispatches the caller-supplied structured payload through the four propose-* writers. **No Anthropic call on the server.** Latency is bounded by the dispatched-handler count, not by an LLM round-trip.

> **Latency note.** UC-14 is synchronous and bounded — there is no LLM call on the server path, so request-response time scales with the number of dispatched `propose_*` handlers (each is a single transaction; the entity-resolution advisory lock of BR-20 serialises identical-name nodes within the call). At v1's single-owner scale this is comfortably sub-second for typical chat-turn-sized payloads (a handful of fragments / nodes / links). The chat agentic loop therefore observes the full ingestion outcome inline on the SSE stream — no polling is required.
...
### BR-14 -- Structural failures map to VALIDATION_* / BUSINESS_UNKNOWN_* / RESOURCE_NOT_FOUND (UC-08..UC-11)
> P2.1 canonical taxonomy. The former §14 short codes `STRUCTURAL_INVALID` / `UNKNOWN_TYPE` / `NOT_FOUND` are deprecated on both transports and MUST NOT appear on the wire.
- Zod structural failures — required field missing / type mismatch / length or range violation / cross-table compatibility mismatch (chunk not in run's source, `key.value_type` mismatch, value_type parse failure) -> `VALIDATION_REQUIRED_FIELD` | `VALIDATION_INVALID_FORMAT` | `VALIDATION_OUT_OF_RANGE` (Zod-discriminated; same discrimination REST already uses).
- `node_type` / `link_type` / `key` not in the seeded catalog -> `BUSINESS_UNKNOWN_NODE_TYPE` | `BUSINESS_UNKNOWN_LINK_TYPE` | `BUSINESS_UNKNOWN_ATTRIBUTE_KEY` (which catalog table the missing name belongs to).
- A referenced `chunk_id` / `fragment_id` / `node_id` that resolves to no row -> `RESOURCE_NOT_FOUND`.
- Anti-hallucination layer 5 rejection (BR-18) also emits `VALIDATION_INVALID_FORMAT` (cited fragment does not anchor a chunk of the run's source).

Parity guard: for every one of these conditions the REST propose-* mirror wire code and the MCP propose-* wire code are byte-identical (the parity tests referenced in the global catalog are the CI gate).

### BR-15 -- Graph-rule failures map to BUSINESS_LINK_RULE_VIOLATION (UC-10)
> P2.1 canonical taxonomy. The former §14 short code `RULE_VIOLATION` is deprecated on both transports.

`propose_link` requires a vigent `LinkTypeRule` row matching `(source_node_type, link_type, target_node_type)`. The 22 seeded rules of §15.2 (`migrations/0001_seed.sql`) plus the 6 added by `migrations/0002_catalog_tier1.sql` (28 total) are the current set. Any other triple yields `BUSINESS_LINK_RULE_VIOLATION` (registered under Ingestion in `docs/specs/_global/error-codes.md`; byte-identical on REST and MCP).

### BR-16 -- Temporal failures map to BUSINESS_TEMPORAL_INCOHERENT / BUSINESS_DATE_UNJUSTIFIED (UC-10, UC-11)
> P2.1 canonical taxonomy. The former §14 short codes `TEMPORAL_INCOHERENT` / `DATE_UNJUSTIFIED` are deprecated on both transports; the namespaced replacements are already registered under Curation in the global catalog and are reused verbatim here.
- `valid_from >= valid_to`, or `change_hint = 'correction'` without a fragment text containing an errata signal -> `BUSINESS_TEMPORAL_INCOHERENT`.
- The type/key requires a `valid_from` and none of `stated` / `document` (`metadata.document_date`) supplies one — and the BFF cannot fall back to `received` because that fallback is reserved for stable cases — -> `BUSINESS_DATE_UNJUSTIFIED`.
...
All three transports invoke the same transport-agnostic service function (`proposeFragmentService`, `proposeNodeService`, `proposeLinkService`, `proposeAttributeService`); the Zod input schemas are the single source of truth, with the MCP/JSON-Schema and the REST request schemas derived via `zod-to-json-schema`. Every invocation must be associated with an `LLMRun` whose `status = 'running'`. The BFF rejects calls without that context with the SAME P2.1 namespaced error.code on both transports: `RESOURCE_NOT_FOUND` (run id unknown) — REST returns HTTP 404, MCP wraps it in the envelope; `BUSINESS_RUN_NOT_RUNNING` (run exists but is not `running`) — REST returns HTTP 409, MCP wraps it in the envelope. The pre-P2.1 MCP behaviour of collapsing both cases into `STRUCTURAL_INVALID` is retired — the LLM sees the same namespaced discriminator a human REST caller sees. This dual-transport design is a deliberate departure from §14 / A28 of v7 (which described `ingest` as MCP-only); the architectural decision is documented in `CLAUDE.md` alongside the Neon deviation.
...
2. **Decide the outcome:**
   - **`consolidated`** — there is a vigent equivalent with the same target/value AND `change_hint = 'none'` AND no textual succession/correction signal in the cited fragments. Three sub-cases, all returning `validation_outcome = 'consolidated'`, no new row, append one `Provenance` row per cited fragment to the existing row (re-affirmation, §18), and preserve the vigent row's `valid_from` / `valid_from_basis` as-is:
     - (i) **Multi-current** — `link_type.allows_multiple_current = true` (or `attribute_key.allows_multiple = true`): consolidate regardless of whether the new proposal's `valid_from` matches the vigent row's. The DB partial dup-guard is keyed by `(source, link_type, target)` / `(node, attribute_key, value)` — `valid_from` is deliberately **not** part of the key. The typical trigger is the per-document `received` fallback (FR-001 in `temporal.ts`) yielding a different `valid_from` on a second document re-asserting the same fact.
     - (ii) **Functional with identical `valid_from`** — `allows_multiple_current = false` (links) / `allows_multiple = false` (attributes), new proposal's `valid_from` equals the vigent row's: classical re-affirmation.
     - (iii) **Functional with divergent `valid_from`** — same scope as (ii) but the new proposal's `valid_from` differs from the vigent row's, AND the new proposal is **not** a different target/value (i.e. succession is not in play) AND no `change_hint = 'correction'` signal: consolidate as well; the divergent date is treated as informational, because succession (alt `3b`) requires a different target/value plus a textual signal. Promotion of `valid_from_basis` (e.g. `received → stated`) on consolidation is out of scope for this revision (§8).
...
The DB partial-unique guards (`knowledge_link_current_dup_guard`, `node_attribute_current_dup_guard`) are no longer reachable in the happy path — consolidation collapses re-affirmations before they become INSERTs. If they ever fire (race with another transaction), the service catches SQLSTATE `23505` on those constraint names and retries the lookup-and-decide step inside the same transaction; on a second failure, the call is rejected with `VALIDATION_INVALID_FORMAT` (P2.1 namespaced — structural layer surface) with a message identifying it as a real concurrent race and the `ToolCall` is recorded with `validation_outcome = 'rejected'`.
...
2. Load a CatalogSnapshot of the live catalog (§15 seed + migration `0002`: 9 NodeTypes, 13 LinkTypes + 28 rules, 16 AttributeKeys) in memory at startup; pass it as part of the SYSTEM prompt. The snapshot is invalidated only by a BFF restart (catalog mutation requires a migration — see §8).
...
> P2.1 (2026-07-02) — canonical namespaced taxonomy. The MCP envelope error.code is byte-identical to the REST wire code for the same business condition; the seven §14 short codes (`STRUCTURAL_INVALID`, `UNKNOWN_TYPE`, `RULE_VIOLATION`, `TEMPORAL_INCOHERENT`, `DATE_UNJUSTIFIED`, `NOT_FOUND`, `INTERNAL`) are deprecated on both transports and MUST NOT appear on the wire once the code consequence lands. Every code below is registered in `docs/specs/_global/error-codes.md`.

| Situation | MCP error.code | Description |
|-----------|----------------|-------------|
| Required field missing, type mismatch, length/range violation, or cross-table compatibility failure (chunk not in run, key/value type mismatch, chunk does not belong to run's source, anti-hallucination BR-18) | `VALIDATION_REQUIRED_FIELD` \| `VALIDATION_INVALID_FORMAT` \| `VALIDATION_OUT_OF_RANGE` | UC-08..UC-11. Zod-discriminated — same discrimination REST already uses. |
| Ambient `llm_run_id` not `running` (MCP path — arg-based binding per BR-21) | `BUSINESS_RUN_NOT_RUNNING` | UC-08..UC-11 alt `1a` (MCP branch). Byte-identical to the REST 409 code. |
| Ambient `llm_run_id` unknown (row does not exist) | `RESOURCE_NOT_FOUND` | UC-08..UC-11 alt `1a` (MCP branch). Byte-identical to the REST 404 code. |
| `node_type` not in the seeded catalog | `BUSINESS_UNKNOWN_NODE_TYPE` | UC-09. |
| `link_type` not in the seeded catalog | `BUSINESS_UNKNOWN_LINK_TYPE` | UC-10. |
| `key` not in the seeded catalog | `BUSINESS_UNKNOWN_ATTRIBUTE_KEY` | UC-11. |
| `propose_link` triple not authorised by any vigent `LinkTypeRule` | `BUSINESS_LINK_RULE_VIOLATION` | UC-10. |
| `valid_from >= valid_to`, or `change_hint = 'correction'` without errata signal | `BUSINESS_TEMPORAL_INCOHERENT` | UC-10, UC-11. |
| `requires_valid_from = true` and none of `stated` / `document` supplies a date | `BUSINESS_DATE_UNJUSTIFIED` | UC-10, UC-11. |
| Referenced `chunk_id` / `fragment_id` / `node_id` does not exist | `RESOURCE_NOT_FOUND` | UC-08..UC-11. |
| Unhandled internal exception in service layer | `SYSTEM_INTERNAL_ERROR` | UC-08..UC-11. |
...
> **Directed ingestion (UC-14) reuses the P2.1 namespaced envelope codes.** `ingest_directed` does not introduce a new MCP error code: Zod failure -> `VALIDATION_REQUIRED_FIELD` / `VALIDATION_INVALID_FORMAT` / `VALIDATION_OUT_OF_RANGE` (P2.1 Zod-discriminated); intake failure -> `SYSTEM_SERVICE_UNAVAILABLE` (when `isPgUnavailable`) or `SYSTEM_INTERNAL_ERROR` (with a generic message; the raw `err.message` is logged server-side only). Per-item `propose_*` rejections are forwarded verbatim into `result.report[i]` and are NOT surfaced as top-level MCP error codes — the top-level envelope stays `{ ok: true, ... }` on the success path even when individual items rejected (BR-34 "Error returned"). The retired UC-13 (`start_async_ingestion`) similarly introduced no new MCP error code; its detached-orchestrator failure path is no longer reachable in v1.4.0.
...
- **Worker-queue offload of extraction.** UC-12 is synchronous and LLM-bound. The retired UC-13 (`start_async_ingestion`, BR-32 WITHDRAWN in v1.4.0) previously offered a same-process detached-promise variant so the CALLER did not block; it has been replaced on the chat path by UC-14 (`ingest_directed`, BR-34), which is synchronous-but-bounded because it runs no LLM on the server. Neither the retired UC-13 nor UC-14 is a worker queue (the BFF process still owns every orchestrator lifetime, restarts orphan any in-flight run, no horizontal scaling). A true worker queue / job system with cross-process durability is a permanent-future concern at the v1.0.0 scale (single-owner, hundreds of documents) and is not part of this version. Likewise, a boot-time sweeper that auto-fails stale `running` runs orphaned by a BFF restart is out of scope (historical context: formerly documented under UC-13 alt 5a; UC-14 has no equivalent orphan hazard because its dispatched-handler latency is sub-second).
...
| 1.2.1 | 2026-06-14 | Spec Writer | correction | UC-10 alt-3a and BR-25 step 2 (`consolidated` outcome) rewritten to fix the deterministic mis-rejection of cross-document re-affirmation. **Root cause:** the previous rule required identical `valid_from` between the vigent row and the new proposal in every consolidation case, which broke §18 "re-affirmation consolidates, never duplicates" whenever the second document's `valid_from_basis = 'received'` fallback (FR-001 in `temporal.ts`) yielded a different per-document date than the vigent row — the consolidator skipped the re-affirmation branch and the call ended up hitting the partial dup-guard `knowledge_link_current_dup_guard` (which is scoped by `(source, link_type, target)` and intentionally excludes `valid_from`). **Fix:** the consolidated branch is now split into three sub-cases — (i) multi-current (`allows_multiple_current = true` for links, `allows_multiple = true` for attributes): consolidate when same target/value + `change_hint = 'none'`, ignore `valid_from` divergence; (ii) functional with identical `valid_from`: previous behaviour preserved; (iii) functional with divergent `valid_from` but same target/value and no succession/correction signal: also consolidated (succession requires a **different** target/value plus a textual signal — a divergent date alone is informational). The vigent row's `valid_from` / `valid_from_basis` are preserved as-is on consolidation in all three sub-cases; promotion of a stronger evidence basis (`received → stated`) is out of scope (§8). Aligned with `ingestion.back.md` v1.2.2 BR-27 (the implementation contract — sub-cases (i)/(ii)/(iii) of BR-25 step 2 here map 1-1 onto BR-27 Re-affirmation sub-cases (i)/(ii)/(iii)). No new BR; no schema change; no OpenAPI surface change (the `consolidated` outcome and `validation_outcome` enum already cover the corrected behaviour). No new BUSINESS_ error code. | fix-link-reaffirmation-consolidation |
| 1.2.2 | 2026-06-17 | Maintainer | update | Added a note to **UC-12** documenting the MCP one-shot trigger `ingest_document`: an external MCP client (e.g. Claude Desktop) performs UC-01 + UC-12 in a single tool call (creates `RawInformation` + chunks + `running` `LLMRun`, then runs the server-side extraction orchestrator and returns the run summary; idempotent via `content_hash` → `already_ingested`, no re-extraction). The extraction LLM is the server's (`ANTHROPIC_API_KEY`); MCP-facing only (REST keeps UC-01 and UC-12 as separate endpoints); same synchronous latency profile as UC-12. Descriptive only — composes two existing UCs over the existing orchestrator and service layer. No new UC / BR / ST; no schema change; no new error code. Implementation contract in `ingestion.back.md` v1.2.5 **BR-30**; reconciled into the normative source by **Emenda v7.4** (§14.1, §14, A28). | claude-desktop-mcp |
| 1.3.0 | 2026-06-22 | Spec Writer | update | Added **UC-13 `start_async_ingestion`** — a descriptive composition of UC-01 (intake) + UC-12 (extraction) where the orchestrator is fired as a detached background promise and the caller awaits ONLY the intake. Distinct from `ingest_document` (BR-30 — synchronous composition, awaits the full extraction) and from `get_ingestion_status` (BR-31 — read-only poller). Same argument shape and idempotent `already_ingested` semantics as `ingest_document`; the new-run path differs (returns `run_status: 'running'` immediately; the caller polls `get_ingestion_status` for the terminal status). MCP-facing only — there is no REST mirror, because the REST surface already exposes UC-01 and UC-12 as separate endpoints (a REST caller wanting async simply skips the UC-12 call). Reachable over BOTH MCP transports (HTTP + local stdio). Rollout-flag guarded (`env.CHAT_INGEST_ENABLED`, default `false`): when off, the tool is not registered at boot; UC-13 alt 1a documents the dark-launch path. UC-13 alt 5a documents the BFF-restart orphan condition (the run stays `running`, recovered by the Owner via REST UC-06 + UC-12; a boot-time sweeper is out of scope, §8). Updated §6.2 with a clarifying note that UC-13 reuses the existing MCP envelope codes (`STRUCTURAL_INVALID` / `SYSTEM_SERVICE_UNAVAILABLE` / `INTERNAL`); extraction-side failures on the detached orchestrator are NOT surfaced as MCP error codes on the UC-13 response — only via the next poll on `llm_run.status`. Updated §8 — the previous "Asynchronous extraction" out-of-scope bullet is refined to acknowledge UC-13 (same-process detached promise) while keeping a true cross-process worker queue and a boot-time stale-`running` sweeper out of scope. Implementation contract in `ingestion.back.md` v1.2.9 **BR-32** (CHAT_INGEST_ENABLED flag, detached-promise fire-and-forget, chat-module integration via `mcp.getTool('ingest','start_async_ingestion')`, NOT a member of `CHAT_TOOL_NAMES`). No new BUSINESS_ error code. No schema change. No OpenAPI surface change (MCP-only tool; `openapi.yaml` description updated descriptively to mention UC-13 alongside UC-12 / BR-30 / BR-31). | chat-async-ingestion |
| 1.4.0 | 2026-06-25 | Spec Writer | update | Retired **UC-13 `start_async_ingestion`** and added **UC-14 `ingest_directed`** — deterministic, synchronous directed-ingestion composition. UC-13 is marked RETIRED with a traceability placeholder (the UC number is **reserved**, not reused, so historical references in the back-spec and audit logs remain unambiguous); the sole consumer of UC-13 (the chat agentic loop) now uses UC-14 instead. UC-14 documents the chat-driven path where the LLM structures the turn context client-side and the server runs **no LLM**: the directed orchestrator opens an `LLMRun` with sentinels `model='directed'` / `prompt_version='directed-v1'`, dispatches the four `propose_*` handlers in dependency order (fragments —> nodes —> attributes —> links) under the standard five-layer validation pipeline (BR-13..BR-18, BR-24, BR-25), applies a per-item cascade rule for failed `*_ref` dependencies, and returns a synchronous per-item report inline. Rollout is gated for the chat consumer by `env.CHAT_INGEST_ENABLED` (boot-only); the tool itself is always registered on the `ingest` toolset for direct external MCP callers. Per-call nonce in the synthesised content guarantees `content_hash` uniqueness — there is no `noop_existing` branch on this path; re-affirmation is handled at the graph layer by BR-25 (`consolidated`). Updated the UC-12 trailing note (the previous "MCP async trigger" sidebar now points to the directed-ingestion sidebar for UC-14). Implementation contract in `ingestion.back.md` v1.4.0 / v1.4.1 **BR-34** (BR-32 marked WITHDRAWN, reserved). No new BUSINESS_ error code (Zod -> `STRUCTURAL_INVALID`; intake -> `SYSTEM_SERVICE_UNAVAILABLE` / `INTERNAL`; per-item rejections forwarded verbatim into `result.report[i]`). No schema change. No REST surface change (MCP-only tool); `openapi.yaml` v1.4.1 updates the description block to reference `ingest_directed` and the retirement of `start_async_ingestion`. | retire-async-ingest-add-directed |
| 1.5.0 | 2026-07-02 | Spec Writer | update | **P2.1 landing (companion to the compliance-audit + global-catalog wave, 2026-07-02).** Reconciled `ingestion.spec.md`, `ingestion.back.md` and `openapi.yaml` with the owner-approved canonical namespaced error-code taxonomy. Every §14 short code emitted by this domain migrates on both transports (REST propose-* mirrors AND MCP `ingest` toolset) to its P2.1 namespaced replacement: `STRUCTURAL_INVALID` -> `VALIDATION_REQUIRED_FIELD` / `VALIDATION_INVALID_FORMAT` / `VALIDATION_OUT_OF_RANGE` (Zod-discriminated); `UNKNOWN_TYPE` -> `BUSINESS_UNKNOWN_NODE_TYPE` / `BUSINESS_UNKNOWN_LINK_TYPE` / `BUSINESS_UNKNOWN_ATTRIBUTE_KEY` (catalog-discriminated — new for the ingestion domain, but the three replacement codes are already registered under Knowledge Graph in the global catalog); `RULE_VIOLATION` -> `BUSINESS_LINK_RULE_VIOLATION` (new code, registered by this landing wave under Ingestion in the global catalog); `TEMPORAL_INCOHERENT` -> `BUSINESS_TEMPORAL_INCOHERENT` (reused verbatim from Curation); `DATE_UNJUSTIFIED` -> `BUSINESS_DATE_UNJUSTIFIED` (reused verbatim from Curation); `NOT_FOUND` -> `RESOURCE_NOT_FOUND`; `INTERNAL` -> `SYSTEM_INTERNAL_ERROR`. The pre-P2.1 MCP behaviour of collapsing the not-running / unknown-run cases into `STRUCTURAL_INVALID` is retired — the MCP wire now surfaces `BUSINESS_RUN_NOT_RUNNING` and `RESOURCE_NOT_FOUND` byte-identically to the REST wire (BR-21). Touched sections: UC-08 alt 1a/2a/2b/2c; UC-09 flow step 2 + alt 2a/2b; UC-10 alt 2a..2f; UC-11 alt 2g/2h; UC-14 step 2 + alt 2a/3a/4a; BR-14 heading + body rewritten; BR-15 heading + body rewritten; BR-16 heading + body rewritten; BR-18 code updated (layer 5 emits `VALIDATION_INVALID_FORMAT`, joining layer 1 in the structural bucket); BR-21 rejection table rewritten; BR-25 correction branch + dup-guard fall-through code updated; §6.2 MCP envelope error table rewritten (10 rows — the ambient-run-not-running case is now split into two distinct namespaced codes; each catalog-miss situation has its own row); UC-14 directed-note updated. **HTTP-semantics rule (unified per P2.1):** business errors are never HTTP errors on either transport; the REST propose-* mirrors continue to return `200 OK` with the MCP envelope shape for layered-validation rejections (owner-approved); real HTTP 4xx/5xx stay reserved for auth/transport (`AUTH_*`, `SYSTEM_SERVICE_UNAVAILABLE`, `SYSTEM_LLM_PROVIDER_UNAVAILABLE`) and pre-handler REST validation (`422 VALIDATION_*`, `404 RESOURCE_NOT_FOUND`, `409 BUSINESS_RUN_NOT_RUNNING`). **Parity guard:** the REST<->MCP wire-parity assertion becomes a first-class test contract (BR-13..BR-18 parity — companion to compliance BR-14, curation BR-32, knowledge-graph TC-04) — byte-identical namespaced codes across the two transports for every business condition. **Spec-first, no code change in this landing:** the code consequence (updating `backend/src/shared/error-mapping.ts` as the single registry, removing the `code/mcpCode` pair from `backend/src/modules/ingestion/validation/errors.ts`, migrating the ingestion MCP transport handler mapper, and asserting the parity tests) lands in a separate PR; until that PR merges, the ingestion MCP wire is in a transitional state (the seven short codes may still appear briefly). No new use case, no new endpoint, no new REST route, no schema change, no DDL, no new MCP tool. The `openapi.yaml` companion (bumped to v1.5.0) updates the `ProposeMcpEnvelope.error.code` enum from the seven §14 short codes to the P2.1 namespaced set, updates every propose-* example to the namespaced code, and rewrites the `RunNotRunning` response description to note the MCP wire byte-identity. | p2-1-error-code-taxonomy |
| 1.5.1 | 2026-07-02 | Spec Writer | correction | **P2.1 repair (companion to `ingestion.back.md` v1.5.1).** Surgical fix: UC-12 alt `4b` inner `tool_result` envelope example still referenced the deprecated §14 short code `INTERNAL` in the LLM-facing rejection payload — updated to the canonical namespaced code `SYSTEM_INTERNAL_ERROR`, aligning the spec-side text with the back-spec (BR-26) and the P2.1 taxonomy already rolled out in 1.5.0 elsewhere in this document. No other section touched; the surrounding sentence (fatal-burst outer HTTP mapping to `500 SYSTEM_INTERNAL_ERROR`) was already correct. No new UC / BR / error code / schema / OpenAPI change. | p2-1-error-code-taxonomy |
