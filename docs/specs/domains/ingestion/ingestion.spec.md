...
**Main flow:**
1. Caller invokes `propose_fragment { text, confidence, chunk_ids }` (MCP tool call, REST POST `/api/v1/ingest/llm-runs/{llmRunId}/propose-fragment`, or in-process tool-use dispatch from UC-12).
...
4. For each `RawChunk` of the run's source, in `chunk_index` order:
   ...
   2. Open an Anthropic SDK message stream (`client.messages.stream(...)`) with the four `ingest` tools (`input_schema` derived from the same Zod schemas used by MCP and REST via `zod-to-json-schema` — BR-21) and `thinking: { type: "adaptive" }`.
