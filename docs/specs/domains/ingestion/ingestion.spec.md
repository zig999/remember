...
2. BFF Zod-parses the input. Failure -> envelope `{ ok: false, error.code: "STRUCTURAL_INVALID" }`; no run row is opened, no `tool_call` row is written (there is nothing to audit against yet).
...
- `2a` Zod parse failure on the input -> `{ ok: false, error.code: "STRUCTURAL_INVALID" }`. No `RawInformation` is written; no `tool_call` row.
- `3a` Intake failure (UC-01 alt 5a — chunker error, DB outage, schema violation, etc.) -> the handler catches and returns `{ ok: false, error.code: "SYSTEM_SERVICE_UNAVAILABLE" }` when `isPgUnavailable(err)`, otherwise `{ ok: false, error.code: "INTERNAL" }` with a generic message (raw `err.message` logged server-side only, never returned). No partial run exists. No `tool_call` row.
- `4a` A dispatched `propose_*` returns a rejection envelope (`STRUCTURAL_INVALID`, `UNKNOWN_TYPE`, `RULE_VIOLATION`, `TEMPORAL_INCOHERENT`, `DATE_UNJUSTIFIED`, `NOT_FOUND`, `BELOW_CONFIDENCE_FLOOR`) -> the rejection is **forwarded verbatim** into `result.report[i]`; the top-level envelope stays `{ ok: true, ... }`; the run still closes `completed`; a `tool_call` row is still written for that handler invocation with `validation_outcome = 'rejected'` (BR-23).
...
`propose_link` requires a vigent `LinkTypeRule` row matching `(source_node_type, link_type, target_node_type)`. The 22 seeded rules of §15.2 (`migrations/0001_seed.sql`) plus the 6 added by `migrations/0002_catalog_tier1.sql` (28 total) are the current set. Any other triple yields `RULE_VIOLATION`.
...
> **Directed ingestion (UC-14) reuses the existing envelope codes.** `ingest_directed` does not introduce a new MCP error code: Zod failure -> `STRUCTURAL_INVALID`; intake failure -> `SYSTEM_SERVICE_UNAVAILABLE` (when `isPgUnavailable`) or `INTERNAL` (with a generic message; the raw `err.message` is logged server-side only). Per-item `propose_*` rejections are forwarded verbatim into `result.report[i]` and are NOT surfaced as top-level MCP error codes — the top-level envelope stays `{ ok: true, ... }` on the success path even when individual items rejected (BR-34 "Error returned"). The retired UC-13 (`start_async_ingestion`) similarly introduced no new MCP error code; its detached-orchestrator failure path is no longer reachable in v1.4.0.
