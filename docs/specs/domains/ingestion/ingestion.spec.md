...
> **MCP async trigger — `start_async_ingestion` (`ingestion.back.md` BR-32).** Distinct sibling of `ingest_document`: same arguments, same idempotent `already_ingested` semantics, but on the new-run path the tool DOES NOT await extraction — it fires the orchestrator as a detached promise and returns immediately. UC-13 below documents this composition. The two MCP tools coexist intentionally — the synchronous one is the right answer for single-shot CLI / batch ingestion (one round-trip carries the full result); the asynchronous one is the right answer for callers that cannot afford to block (chat agentic loop, `mcp-remote` with short client timeouts).

### UC-13 -- Start an asynchronous ingestion (intake now, extract in background)

**Actor:** External MCP client (the chat agentic loop, or any MCP-capable Owner-side client). Internally composes UC-01 (intake) and UC-12 (extraction), but the caller awaits only UC-01.
**Pre:** Caller is authenticated (HTTP MCP transport: Neon Auth JWT verified by `requireNeonAuth`; local stdio MCP transport: local-process trust boundary per `knowledge-graph.back.md` BR-01 stdio carve-out). `content` is a non-empty UTF-8 string ≤ 10 MiB. `ANTHROPIC_API_KEY` is configured on the BFF. **The `env.CHAT_INGEST_ENABLED` rollout flag is `true`** — when the flag is `false`, the tool is not registered at boot and the call surfaces as the MCP "tool not found" path (the chat agentic loop falls back gracefully, see `ingestion.back.md` BR-32 "Chat integration").
**Post:** On the new-run path: `RawInformation` + chunks + a `running` `LLMRun` exist (UC-01 has completed in < 1 s). The extraction orchestrator (`runLlmExtraction`, UC-12) has been LAUNCHED as a detached promise in the BFF process — it is running but the caller has not awaited it. The HTTP / stdio response carries `{ outcome: 'ingested', raw_information_id, llm_run_id, chunk_count, run_status: 'running' }`. The orchestrator continues in the background and will set `llm_run.status` to `completed` or `failed` on its own. On the idempotent path: no new rows are written, no background extraction is launched, and the response carries `{ outcome: 'already_ingested', raw_information_id, llm_run_id, chunk_count, run_status }` where `run_status` is the existing run's current `status`. **Polling is the only progress channel** — the caller observes the run via `get_ingestion_status` (`ingestion.back.md` BR-31).

**Main flow:**
1. MCP client invokes tool `start_async_ingestion` with `{ content, source_type, metadata?, model?, prompt_version? }` (identical shape to `ingest_document`).
2. BFF Zod-parses the input. Failure -> envelope `{ ok: false, error.code: "STRUCTURAL_INVALID" }` (no run exists yet, no `ToolCall` row).
3. BFF runs `ingestRawInformation` (UC-01 main flow + alt 4a) in one transaction — persists `RawInformation` + chunks + opens the `running` `LLMRun`.
4. If UC-01 returned `outcome = 'noop_existing'` (idempotent path, BR-08/BR-09): BFF best-effort reads the existing run's `status` and returns `{ ok: true, result: { outcome: 'already_ingested', raw_information_id, llm_run_id, chunk_count, run_status } }`. **No background extraction is launched** (symmetry with `ingest_document` step 3). End of UC.
5. New-run path (`outcome = 'created'`): BFF builds a detached promise calling `runLlmExtraction(llm_run_id)` (UC-12 main flow), wraps it in a top-level `.catch(err => logger.error(...))` (prevents `unhandledRejection`), and **does NOT await** it.
6. BFF returns immediately with `{ ok: true, result: { outcome: 'ingested', raw_information_id, llm_run_id, chunk_count, run_status: 'running' } }`. Total latency from request to response is dominated by the UC-01 intake (< 1 s on a typical Neon link).
7. The detached orchestrator continues per UC-12 (per-chunk Anthropic SDK tool-use loop, per-tool-call transactions, terminal `completed` or `failed` write). The caller polls `get_ingestion_status` (BR-31) to learn the terminal status; the `summary` derived from `tool_call.validation_outcome` (BR-12) becomes available as the orchestrator commits each tool call.

**Alternative flows:**
- `1a` `env.CHAT_INGEST_ENABLED = false` at BFF boot -> tool is not registered; the MCP client gets the SDK's standard "tool not found" envelope. This is the dark-launch state.
- `2a` Zod parse failure on the input -> `STRUCTURAL_INVALID` envelope. No `RawInformation` is written.
- `3a` Intake error (UC-01 alt 5a — chunker error, DB outage, schema violation) -> the handler catches the error and returns `{ ok: false, error.code: "SYSTEM_SERVICE_UNAVAILABLE" }` when `isPgUnavailable(err)`, otherwise `{ ok: false, error.code: "INTERNAL" }` with a generic message (the raw `err.message` is logged server-side only, never returned). No background extraction is launched. No partial run exists.
- `4a` Idempotent re-send of content whose first run is `failed` -> the response is `{ outcome: 'already_ingested', ..., run_status: 'failed' }` with an explicit fail-loud `message`. **No background extraction is launched** (the contract is symmetric with `ingest_document` — re-sending does NOT re-attempt extraction). Recovery is the same as for `ingest_document`: the Owner uses REST `POST /llm-runs/:id/retry` (UC-06) followed by `POST /llm-runs/:id/run` (UC-12). A dedicated MCP retry/re-run tool is **out of scope** for this revision.
- `5a` BFF process restarts while the detached orchestrator is mid-run -> the `llm_run.status` stays `running` (a known-orphan condition). The owner detects it by polling `get_ingestion_status` and recovers via `retryLlmRun` (BR-10 atomically flips `failed → running` and clears orphan fragments — note however that for an orphaned `running` run the owner must first force the row to `failed` out of band, since BR-11 forbids retry of a still-`running` row; a sweeper that auto-fails stale `running` runs at boot is **out of scope**, see §8).
- `6a` The detached orchestrator hits a fatal Anthropic error (UC-12 alt 4a) -> the orchestrator closes the run as `failed` and (per the detached-promise contract) logs at ERROR level. The caller never sees a 502 — the caller has already received `{ outcome: 'ingested', run_status: 'running' }` at step 6. The next poll on `get_ingestion_status` returns `run_status: 'failed'` with the partial summary (BR-12). No `SYSTEM_LLM_PROVIDER_UNAVAILABLE` envelope is returned to UC-13 — that code is reserved for the synchronous extraction triggers (UC-12 alt 4a, BR-30 `ingest_document` extraction-fatal path).
- `6b` The detached orchestrator hits a non-Anthropic fatal exception (UC-12 alt 4b) -> same observability rule as `6a`: caller observes `run_status: 'failed'` on the next poll; no `SYSTEM_INTERNAL_ERROR` envelope is returned to UC-13.

**Related endpoint / tool:** MCP tool `start_async_ingestion` on the `ingest` toolset (advertised by both MCP transports — HTTP `POST /api/v1/mcp/ingest` and local stdio `backend/src/mcp-stdio.ts`). **No REST mirror** — the REST surface already exposes UC-01 (`POST /raw-information`) and UC-12 (`POST /llm-runs/:id/run`) as separate endpoints; a REST caller that wants async behaviour calls UC-01 and skips UC-12.

> **Distinction from UC-12 and `ingest_document` (BR-30).**
> - **UC-12 `runLlmExtraction`**: REST + synchronous, operates over an existing `running` `LLMRun` (the caller owns the run; usually the Owner via the SPA). Blocks for the whole extraction; returns the run summary.
> - **`ingest_document` (BR-30 / UC-01 + UC-12 sync composition)**: MCP + synchronous, creates the run and drives extraction. Blocks for the whole extraction; returns the run summary. Right for single-shot CLI ingestion.
> - **UC-13 `start_async_ingestion` (BR-32 / UC-01 + UC-12 async composition)**: MCP + asynchronous, creates the run and **fires-and-forgets** extraction. Returns `run_status: 'running'` immediately; outcomes are observed via `get_ingestion_status` (BR-31). Right for the chat agentic loop and for any MCP client that cannot afford to block.

> **Latency note.** UC-13 trades extraction-time visibility for caller latency. The intake step (< 1 s) is bounded by Neon round-trip; the orchestrator that runs in the background is LLM-bound (minutes per document on typical content, same as UC-12 / BR-30 sync). The caller's request-response is therefore O(intake), not O(extraction). At v1's single-owner scale this is acceptable; a future evolution (worker queue, push-based progress channel) is **out of scope** (§8 — same line that already excludes worker offload of UC-12).
...
> **Async ingestion (UC-13) reuses the existing envelope codes.** `start_async_ingestion` does not introduce a new MCP error code: Zod failure -> `STRUCTURAL_INVALID`; intake failure -> `SYSTEM_SERVICE_UNAVAILABLE` (when `isPgUnavailable`) or `INTERNAL`. Extraction-side failures on the detached orchestrator (the equivalents of UC-12 alt 4a/4b) are NOT surfaced as MCP error codes here — they are observable only via the next `get_ingestion_status` poll as `llm_run.status = 'failed'` with the partial `summary` (BR-12, `ingestion.back.md` BR-32 Error returned).
...
- **Worker-queue offload of extraction.** UC-12 is synchronous and LLM-bound. UC-13 (`start_async_ingestion`, BR-32) introduces a same-process detached-promise variant so the CALLER does not block; this is **not** a worker queue (the BFF process still owns the orchestrator's lifetime, restarts orphan the run, no horizontal scaling). A true worker queue / job system with cross-process durability is a permanent-future concern at the v1.0.0 scale (single-owner, hundreds of documents) and is not part of this version. Likewise, a boot-time sweeper that auto-fails stale `running` runs orphaned by a BFF restart is out of scope (UC-13 alt 5a).
