...
> Stack: Node.js 20 LTS + TypeScript strict + Fastify | DB: PostgreSQL 17 via Neon (driver `pg` raw, connection via `DATABASE_URL`) | Version: 1.5.0 | Status: draft | Layer: permanent
...
### BR-24 -- MCP error envelope is produced by a shared mapper extracted from the REST error handler
**Related UC:** all
**Where to validate:** service / shared util -- the existing REST routes contain inline mappers that translate a thrown `pg` error or a known business error (`BUSINESS_NODE_DELETED`, `BUSINESS_UNKNOWN_NODE_TYPE`, `BUSINESS_UNKNOWN_LINK_TYPE`, `BUSINESS_INVALID_TRAVERSE_DEPTH`, `BUSINESS_UNKNOWN_ATTRIBUTE_KEY`, `RESOURCE_NOT_FOUND`, `VALIDATION_INVALID_FORMAT`, `VALIDATION_OUT_OF_RANGE`, `SYSTEM_INTERNAL_ERROR`, `SYSTEM_SERVICE_UNAVAILABLE`) into the REST envelope `{ ok: false, error: { code, message, details } }`. This BR REQUIRES extracting that mapper into a single module (`backend/src/modules/knowledge-graph/mcp/error-envelope.ts`, exported across domains) that converts a thrown exception OR a `Result<ok=false, ...>` into the MCP envelope `{ ok: false, error: { code, message, details } }` -- the same shape as the REST envelope (the only difference is the transport framing). The new MCP query transport (BR-23) MUST consume this mapper; the existing REST handlers MUST be refactored to consume it too (so a service-throw produces the same code on both surfaces -- structural parity, BR-26).
...
A service throw NEVER produces `INTERNAL_ERROR` for a known business condition -- the mapper guarantees a typed code is surfaced. This BR is a regression guard against the prior behaviour where MCP transports collapsed every throw into a generic internal error.
...
**Where to validate:** tests -- per-tool integration tests in `backend/src/__tests__/integration/knowledge-graph/mcp-query-*.spec.ts` MUST issue the same logical request twice (once over REST `GET /api/v1/...`, once over MCP `POST /api/v1/mcp/query` with `tools/call name=<tool>`) against the same seeded fixture and assert (a) the response payload is byte-for-byte identical **after stripping the REST `{ ok: true, result }` envelope on the REST side and the MCP `content` framing on the MCP side** (BR-27 explicitly carves the per-transport framing out of the parity assertion -- only the service-layer return value is compared), (b) on a forced-error case (e.g., unknown node id), the surfaced `code` matches between transports (BR-24 mapper). The unit tests on tools live alongside the toolset module (`mcp/query-toolset.spec.ts`). The functional E2E harness `temp/e2e/query-e2e.mts` is extended with at least one parity assertion: `search` via MCP against a seeded `Person` node returns the same nodes as `search` via REST.
