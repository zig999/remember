...
> Stack: Node.js 20 LTS + TypeScript strict + Fastify | DB: PostgreSQL 17 via Neon (driver `pg` raw, connection via `DATABASE_URL`) | Version: 1.4.0 | Status: draft | Layer: permanent
...
**Error returned:** HTTP 401 -- error.code: `AUTH_UNAUTHORIZED` / `AUTH_TOKEN_INVALID` / `AUTH_TOKEN_EXPIRED` (all registered). The dev carve-out adds no new code (it either authenticates or falls through to these).
...
The tool's output shape is the same JSON the REST handler would return (the service-layer return value, serialised by `JSON.stringify`); the MCP envelope wraps it as `{ ok: true, result: { ... } }`.
