...
> Stack: Node.js 20 LTS + TypeScript strict + Fastify | DB: PostgreSQL 17 via Neon (driver `pg` raw, connection via `DATABASE_URL`) | Version: 1.2.0 | Status: draft | Layer: permanent
...
| MCP server | Same BFF process. **Two distinct MCP transports** (ADR A28, single core / two transports): (i) the pre-existing **ingest** transport `POST /api/v1/mcp` (owned by `ingestion`, write-side, audited, requires `X-LLM-Run-Id` per `ingestion.back.md` BR-21/BR-23); (ii) a NEW read-only **query** transport `POST /api/v1/mcp/query` (this domain + `query-retrieval`, BR-23 below). The two share the same `mcp/server.ts` core but live behind different Fastify routes, different request decoders and different audit policies. The query transport hosts the `query` toolset of v7 §14.3 -- operations `get_node`, `traverse`, `get_history_link`, `get_history_attribute`, `get_history_attribute_key`, `list_nodes`, `list_node_types`, `list_link_types`, `list_attribute_keys` (this domain) plus `search`, `get_provenance_link`, `get_provenance_attribute`, `get_provenance_fragment` (delegated to `query-retrieval`). Every tool is mirrored 1:1 in REST; the tool input Zod schema is identical to the REST DTO Zod schema (same module export), so REST↔MCP parity is structural. | New (this domain). |
...
| Transaction policy | Read-only domain -- every route runs inside `withReadOnly(pool, ...)` (the existing helper in `knowledge-graph/routes/knowledge-graph.routes.ts` -- `BEGIN READ ONLY` ... `COMMIT`), which guarantees a stable `current_date` for all derived fields across multiple queries. The **MCP query transport reuses the SAME `withReadOnly` helper** -- every tool dispatch opens its own short read-only transaction on a dedicated pool client and releases on completion (BR-23). MCP tool calls NEVER write `tool_call` audit rows (BR-23 -- audit is the ingest-side rule of `ingestion.back.md` BR-23, scoped to LLM-write surface, not the query-read surface). | New (this domain). |
...
**Description:** Modifying the existing `POST /api/v1/mcp` transport is **explicitly forbidden** by this BR -- the ingest transport carries the audited write path and its test surface is load-bearing for §13 anti-hallucination guarantees. Co-tenanting read tools onto it would force conditional `X-LLM-Run-Id` semantics and conditional audit-row writes, both of which contradict CLAUDE.md "Rule 7 -- Surface Conflicts, Don't Average Them". The two transports share the underlying `mcp/server.ts` core but expose disjoint tool registries on disjoint URLs.
...
- **Two MCP transports coexist on the same BFF process; they MUST stay disjoint.** `POST /api/v1/mcp` is the **ingest** transport (owned by `ingestion`, write-side, audited, requires `X-LLM-Run-Id`). `POST /api/v1/mcp/query` is the **query** transport (this domain + `query-retrieval`, read-only, no run header, no audit row -- BR-23). Modifying the ingest transport to host read tools is rejected at code review; the two transports share only the `mcp/server.ts` core.
