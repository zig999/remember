...
> Stack: Node.js 20 LTS + TypeScript strict + Fastify | DB: PostgreSQL 17 via Neon (driver `pg` raw, `DATABASE_URL`) | Version: 1.2.0 | Status: draft | Layer: permanent
...
5. Reject any attempt to register a tool name outside the **closed whitelist of 8 names**: `list_review_queue`, `resolve_entity_match`, `merge_nodes`, `resolve_dispute`, `confirm_item`, `reject_item`, `correct_item`, `compliance_delete` (BR-31). Any other name -- including ingest-side tool names (`propose_*`, `finalize_run`, `start_run`) and query-side tool names (`get_node`, `traverse`, `search`, `get_provenance_*`, ...) -- is rejected with envelope `{ ok: false, error.code: "NOT_FOUND" }`. The whitelist is built at transport startup from the union of `CURATION_TOOL_NAMES` (this domain) + the singleton `'compliance_delete'` (BR-31).
