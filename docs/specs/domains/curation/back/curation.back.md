...
> Stack: Node.js 20 LTS + TypeScript strict + Fastify | DB: PostgreSQL 17 via Neon (driver `pg` raw, `DATABASE_URL`) | Version: 1.4.0 | Status: draft | Layer: permanent
...
**The §14 canonical set is the base; curation extends it.** The v7 §14 canonical MCP error codes are `STRUCTURAL_INVALID`, `UNKNOWN_TYPE`, `RULE_VIOLATION`, `TEMPORAL_INCOHERENT`, `DATE_UNJUSTIFIED`, `NOT_FOUND`, `INTERNAL`. The MCP curation transport surfaces a SUPERSET: the canonical codes are subsumed by the richer REST taxonomy (`NOT_FOUND` -> `RESOURCE_NOT_FOUND`, `TEMPORAL_INCOHERENT` -> `BUSINESS_TEMPORAL_INCOHERENT`, `DATE_UNJUSTIFIED` -> `BUSINESS_DATE_UNJUSTIFIED`, `INTERNAL` -> `SYSTEM_INTERNAL_ERROR`, `STRUCTURAL_INVALID` -> `VALIDATION_INVALID_FORMAT`) plus the curation-specific `BUSINESS_NODE_DELETED`, `BUSINESS_REVIEW_NOT_PENDING`, `BUSINESS_INVALID_TARGET_NODE`, `BUSINESS_TARGET_NODE_REQUIRED`, `BUSINESS_SELF_MERGE_FORBIDDEN`, `BUSINESS_ITEM_NOT_DISPUTED`, `BUSINESS_ITEM_NOT_UNCERTAIN`, `BUSINESS_ITEM_NOT_DELETABLE`, `BUSINESS_DISPUTE_WINNER_REQUIRED`, `BUSINESS_DISPUTE_PERIODS_REQUIRED`, `BUSINESS_CORRECTION_NO_CHANGES`, `BUSINESS_REASON_REQUIRED`. The reconciliation in `remember-modelagem-v7.md` §14 / A28 records this: curation, like query, surfaces the richer REST code set on MCP (not the v7 §14 7-code base), because the LLM acting through MCP is a co-equal actor with the SPA Owner and benefits from the same diagnostic precision (`.spec.md` §3 actors table). `compliance_delete` is the single exception that keeps its §14 canonical mapping (BR-31), because its existing implementation in `compliance-audit.back.md` BR-15 already publishes the §14 codes and the compliance-delete surface is intentionally minimal.
...
**`compliance_delete` is the eighth tool, registered separately by `compliance-audit`.** `backend/src/modules/compliance-audit/mcp/compliance-toolset.ts` (existing) is unchanged in behaviour: it stays envelope-producing, keeps its §14 canonical code mapping (`STRUCTURAL_INVALID` / `NOT_FOUND` / `INTERNAL` -- see `compliance-audit.back.md` BR-15), and registers under the `curation` toolset key on the shared `McpServer` instance. The boot-time wiring in `app.ts` (BR-29) intersects the closed whitelist of 8 names against the shared `McpServer` registry to build the `tools/list` response.
...
4. **Whitelist enforcement** -- a `tools/call` with `name=propose_node` (ingest tool) or `name=get_node` (query tool) on the curation transport returns `{ ok: false, error.code: "NOT_FOUND" }` (BR-29 rule 5).
5. **`compliance_delete` parity** -- the REST `POST /api/v1/compliance/deletions` and the MCP `tools/call name=compliance_delete` produce the same `ComplianceDeletion` row and the same `outcome` discriminated union (`deleted` / `noop_already_deleted`); error mappings continue to use the §14 canonical codes per `compliance-audit.back.md` BR-15 (the only tool on the curation transport that uses the canonical code set rather than the rich REST taxonomy).
...
**Related UC:** N/A (additive read-only endpoint -- no UC in `.spec.md`; consumed by the front spec `front/features/curadoria.feature.spec.md` R1 `MetricsStrip`).
**OperationId:** `getCurationMetrics` (per `openapi.yaml` line 146; route `GET /api/v1/curation/metrics`).
...
- **`compliance_delete` keeps its §14 canonical code mapping; the other seven tools use the rich REST taxonomy on MCP.** This is the deliberate asymmetry recorded in BR-30: `compliance_delete` already publishes `STRUCTURAL_INVALID` / `NOT_FOUND` / `INTERNAL` (per `compliance-audit.back.md` BR-15) and the existing tool / spec / SDK clients depend on it; changing those codes would be a breaking change. The other seven tools have no production MCP surface yet, so they inherit the richer REST taxonomy from the outset (`BUSINESS_*`, `VALIDATION_*`, `RESOURCE_NOT_FOUND`, `SYSTEM_*`).
