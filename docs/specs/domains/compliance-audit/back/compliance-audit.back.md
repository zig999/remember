...
> Stack: Node.js 20 LTS + TypeScript strict + Fastify | DB: PostgreSQL 17 via Neon (managed Postgres, driver `pg` raw) | Version: 1.4.0 | Status: draft | Layer: permanent
...
| Validation library | Zod v4 — every REST DTO and the MCP `compliance_delete` input has a Zod schema. Failed Zod parse on REST → 422 with one of `VALIDATION_REQUIRED_FIELD` / `VALIDATION_INVALID_FORMAT` / `VALIDATION_OUT_OF_RANGE`. Failed Zod parse on MCP → envelope `{ ok:false, error.code: <same namespaced code as REST> }` at HTTP 200 with `isError:true` (MCP 2025-06-18) — the exact namespaced discriminator (`VALIDATION_REQUIRED_FIELD` / `VALIDATION_INVALID_FORMAT` / `VALIDATION_OUT_OF_RANGE`) is byte-identical to the REST rendering (BR-14, BR-15 under P2.1). The pre-P2.1 §14 short code `STRUCTURAL_INVALID` is no longer emitted by this domain on any transport (deprecated in `docs/specs/_global/error-codes.md`). | CLAUDE.md default |
...
**Error returned:** HTTP 422 -- error.code: `VALIDATION_REQUIRED_FIELD` (when absent) or `VALIDATION_OUT_OF_RANGE` (when empty-after-trim or > 1000 chars). Under P2.1 the MCP envelope carries the SAME namespaced code byte-identically (no §14 short-code translation) at HTTP 200 with `isError:true` (BR-14, BR-15).
...
**Error returned:** No direct error for the rule itself. A failure within the TX surfaces as UC-01 alt `9a` → `SYSTEM_INTERNAL_ERROR` byte-identical on REST (HTTP 500) and MCP (envelope, HTTP 200) under P2.1 (BR-15).
...
**Error returned:** None directly — failure of the UPDATE surfaces via BR-02 rollback and UC-01 alt `9a` → `SYSTEM_INTERNAL_ERROR` on both transports under P2.1.
...
**Error returned:** None directly. If either INSERT fails, the TX rolls back (BR-02) and UC-01 alt `9a` returns `SYSTEM_INTERNAL_ERROR` on both transports (P2.1).
...
### BR-14 -- compliance_delete is mirrored as MCP tool with shared service layer and byte-identical error codes
**Related UC:** UC-01
**Where to validate:** code structure. The REST handler `routes/compliance.deletions.post.ts` (mounted at `POST /api/v1/compliance/deletions`) and the MCP handler `mcp/compliance-toolset.ts` (registered as the eighth tool on the curation MCP transport `POST /api/v1/mcp/curation` — see `curation.back.md` BR-29 / BR-31) both call into the same `complianceAudit.service.complianceDelete(client, { rawInformationId, reason })` function. Inputs are validated with the same Zod schema (`ComplianceDeleteRequestSchema`). The service is transport-agnostic: it returns a discriminated union `{ outcome: 'deleted' | 'noop_already_deleted', deletion: ComplianceDeletion }`. The REST handler maps `'deleted'` to HTTP 201 and `'noop_already_deleted'` to HTTP 200. The MCP handler wraps both into `{ ok: true, result: <union> }` at HTTP 200 with `isError:false`.

**Transport wiring (v1.2.0 — unchanged in v1.4.0).** The MCP handler is REGISTERED by this domain on the shared `McpServer` registry under the `curation` toolset key (`registerComplianceToolset` in `app.ts`, unchanged). The DISPATCH happens on the `curation` MCP transport — `backend/src/modules/curation/mcp/curation-transport.ts` mounts `POST /api/v1/mcp/curation`, builds its closed whitelist of 8 names from the union of `CURATION_TOOL_NAMES` (the seven curation tools, owned by `curation`) plus the singleton `'compliance_delete'` (owned here), and routes the `tools/call name=compliance_delete` to the handler this domain registered. The transport's `tools/list` advertises `compliance_delete` alongside the seven curation tools via the descriptor the `curation` transport receives at boot.

**Canonical taxonomy (P2.1 — v1.4.0).** The MCP envelope's `error.code` is **byte-identical** to the REST rendering for every business condition. The service surfaces the canonical `ComplianceAuditFailure` union whose `.code` is already one of the namespaced set (`VALIDATION_REQUIRED_FIELD` / `VALIDATION_INVALID_FORMAT` / `VALIDATION_OUT_OF_RANGE` / `RESOURCE_NOT_FOUND` / `SYSTEM_INTERNAL_ERROR`); both handlers reuse the same `renderErrorEnvelope(failure)` helper — the REST handler wraps it into `reply.status(failure.httpStatus).send({ error })`, the MCP handler wraps it into `{ ok:false, error }` with `isError:true` at HTTP 200. There is NO per-transport code override any more — the pre-P2.1 `code/mcpCode` pair in `compliance-audit/service/errors.ts` is retired (the code-consequence PR removes it; see `docs/specs/_global/error-codes.md` P2.1 rollout note step 6). See BR-15 for the exhaustive namespaced code map.

App-wiring: `registerComplianceToolset` (in `app.ts`, unchanged) registers the handler on the `McpServer`; the `curation` domain's bootstrap path (`registerCurationToolset` + `registerCurationMcpTransport`) intersects the closed whitelist of 8 names against the registry. The `ingestionCatalog` guard on the curation MCP transport (`curation.back.md` BR-29 last paragraph) means the curation MCP transport is skipped when the catalog is absent — when that happens, REST `complianceDeleteRawInformation` stays available (it does not depend on the catalog), but the MCP `compliance_delete` tool is unreachable. This is the same lock-step behaviour the curation REST mirror already exhibits.

**Description:** A single business outcome reaches both transports through identical validation, identical DB writes and identical audit rows (ADR A28). The MCP envelope shape is `{ ok: true, result: { outcome, deletion } }` on success / `{ ok: false, error: { code, message, details } }` on failure (§14). The MCP error-code set used by `compliance_delete` is the **canonical namespaced set of BR-15 under P2.1** — byte-identical to REST; the previous §14 short-code asymmetry is deprecated.
**Error returned:** No direct error from the rule. See BR-15 for the canonical namespaced error-code map.

### BR-15 -- Canonical error codes for compliance_delete (P2.1)
**Related UC:** UC-01
**Where to validate:** MCP handler AND REST handler — both consume the SAME `ComplianceAuditFailure` discriminated union raised by the shared service and render it through the SAME `renderErrorEnvelope(failure)` helper. The `failure.code` field is already the canonical namespaced code; there is no per-transport translation table any more.

**Canonical namespaced code map (byte-identical on REST and MCP):**

| Situation | error.code | REST HTTP status | MCP wire |
|-----------|-----------|------------------|----------|
| Zod parse failure — required field missing | `VALIDATION_REQUIRED_FIELD` | 422 | `content` + `isError:true` at HTTP 200 |
| Zod parse failure — invalid format (malformed UUID, malformed RFC 3339) | `VALIDATION_INVALID_FORMAT` | 422 | `content` + `isError:true` at HTTP 200 |
| Zod parse failure — out-of-range (`reason` empty-after-trim or > 1000 chars; `limit` / `offset` bounds) | `VALIDATION_OUT_OF_RANGE` | 422 | `content` + `isError:true` at HTTP 200 |
| `raw_information_id` resolves to no row (UC-01 alt `4a`) | `RESOURCE_NOT_FOUND` | 404 | `content` + `isError:true` at HTTP 200 |
| UC-01 alt `4c` (legacy tombstoned raw without `ComplianceDeletion`) | `SYSTEM_INTERNAL_ERROR` | 500 | `content` + `isError:true` at HTTP 200 |
| Any unhandled exception in service | `SYSTEM_INTERNAL_ERROR` | 500 | `content` + `isError:true` at HTTP 200 |

Auth failures (missing/invalid JWT) are NOT MCP envelope errors — they are produced by the BFF middleware (`requireNeonAuth`) **before** the MCP tool dispatch on the curation MCP transport, and they surface to the MCP client as the standard REST 401 (`AUTH_UNAUTHORIZED`). The MCP envelope is only used for post-dispatch service-layer outcomes (cf. `ingestion.spec.md` §6.2, `curation.back.md` BR-01 last sentence, ADR A28).

**No transport asymmetry any more (P2.1).** The pre-v1.4.0 asymmetry — `compliance_delete` emitting the §14 short codes (`STRUCTURAL_INVALID` / `NOT_FOUND` / `INTERNAL`) on MCP while the seven other curation tools surfaced the rich REST taxonomy — is retired. Every tool on the curation MCP transport (including `compliance_delete`) now surfaces the same canonical namespaced code set through the shared envelope mapper of `curation.back.md` BR-30 (P2.1 removes the last-paragraph carve-out on that spec too). The `code/mcpCode` pair in `backend/src/modules/compliance-audit/service/errors.ts` is retired in the code-consequence PR (`docs/specs/_global/error-codes.md` P2.1 rollout note step 6); the shared envelope is produced through `backend/src/shared/error-mapping.ts` (single source of truth for the code → HTTP-status mapping).

**Deprecated §14 short codes — transition guidance for downstream MCP clients.** Clients written against v1.0.0..v1.3.0 of this spec would look for `STRUCTURAL_INVALID` / `NOT_FOUND` / `INTERNAL` in `error.code`; under P2.1 these values are NEVER emitted any more. The migration is mechanical:
- `STRUCTURAL_INVALID` → one of the three `VALIDATION_*` codes (Zod-discriminated as in REST).
- `NOT_FOUND` → `RESOURCE_NOT_FOUND`.
- `INTERNAL` → `SYSTEM_INTERNAL_ERROR`.

See `docs/specs/_global/error-codes.md` "§14 short-code → namespaced mapping (deprecation table)" for the full registry (all seven §14 short codes across the whole BFF).
...
**Error returned:** One of `VALIDATION_REQUIRED_FIELD` | `VALIDATION_INVALID_FORMAT` | `VALIDATION_OUT_OF_RANGE` | `RESOURCE_NOT_FOUND` | `SYSTEM_INTERNAL_ERROR` per the map above — byte-identical on REST and MCP.
...
Under P2.1 both handlers surface `SYSTEM_INTERNAL_ERROR` byte-identically: the REST route handler maps the throw to HTTP 500 `SYSTEM_INTERNAL_ERROR`; the MCP handler maps it to envelope `{ ok:false, error.code: "SYSTEM_INTERNAL_ERROR" }` at HTTP 200 with `isError:true` (BR-15). The `transport` log field disambiguates REST vs MCP origin for the operator dashboard.
...
**Error returned:** `SYSTEM_INTERNAL_ERROR` on both transports (REST HTTP 500, MCP envelope at HTTP 200 with `isError:true`), byte-identical under P2.1. Always accompanied by the structured log entry above.
...
**Error returned:** None directly — failure of the UPDATE surfaces via BR-02 rollback and UC-01 alt `9a` → `SYSTEM_INTERNAL_ERROR` on both transports (P2.1).
...
```
       compliance_delete (REST POST /api/v1/compliance/deletions
        ...
        [active] ---FOR UPDATE---> read status
              ...
              |                            |
              |                            +- 'deleted' AND ComplianceDeletion missing ---> SYSTEM_INTERNAL_ERROR (REST 500 / MCP envelope, byte-identical)
              v
...
| deleted | — | repeated `complianceDeleteRawInformation` / `compliance_delete` against legacy inconsistency | FOR UPDATE acquires the row; `raw_information.status = 'deleted'` AND `NOT EXISTS (... compliance_deletion ...)`; transaction rolled back; `SYSTEM_INTERNAL_ERROR` on both transports (REST HTTP 500, MCP envelope at HTTP 200 with `isError:true`) — byte-identical under P2.1 (BR-17) | UC-01 alt `4c` |
...
- **P2.1 canonical error-code taxonomy (2026-07-02).** This domain now emits ONE canonical error-code vocabulary — the namespaced set of `docs/specs/_global/error-codes.md` — byte-identically on REST and on the MCP curation transport for `compliance_delete`. The pre-v1.4.0 §14 short codes (`STRUCTURAL_INVALID` / `NOT_FOUND` / `INTERNAL`) are deprecated by that catalog. The `code/mcpCode` pair currently declared in `backend/src/modules/compliance-audit/service/errors.ts` is retired in the code-consequence PR of P2.1 (rollout step 6); the REST/MCP mapping is centralised in `backend/src/shared/error-mapping.ts`. Parity tests (BR-14 Testing row) are the CI guard that keeps the two transports byte-identical. No spec / DB / migration change beyond the wording of BR-14 / BR-15 in this v1.4.0 update.
...
- **`compliance_delete` is co-tenant on the MCP curation transport; the handler stays in `compliance-audit`.** The MCP catalog (§14.4) lists `compliance_delete` inside the `curation` toolset; the curation MCP transport admits it via the closed whitelist of 8 names (`curation.back.md` BR-31) and dispatches to `compliance-audit/mcp/compliance-toolset.ts`. The REST split is intentional for operational clarity (`compliance-audit` REST stays at `/api/v1/compliance/deletions`; the `curation` REST is at `/api/v1/curation/*`). The MCP toolset name `compliance_delete` is unchanged; the error-code mapping is the canonical namespaced set under P2.1 (BR-15 v1.4.0).
- **`compliance_delete` and the other seven curation tools now share ONE canonical error-code vocabulary (P2.1, v1.4.0).** The pre-v1.4.0 asymmetry — `compliance_delete` emitting the §14 short codes (`STRUCTURAL_INVALID` / `NOT_FOUND` / `INTERNAL`) on MCP while the other seven tools surfaced the rich REST taxonomy — is retired. Every tool on the curation MCP transport, including `compliance_delete`, emits the same canonical namespaced set on MCP that it emits on REST (`VALIDATION_*` / `RESOURCE_NOT_FOUND` / `BUSINESS_*` / `SYSTEM_*`). The compliance-audit handler now consumes the shared envelope mapper of `curation.back.md` BR-30 through `backend/src/shared/error-mapping.ts` (single registry, single conversion table — code-consequence PR of P2.1 rollout step 6). Downstream LLM/SDK clients written against the pre-v1.4.0 short codes MUST migrate per the P2.1 deprecation table in `docs/specs/_global/error-codes.md`; the migration is mechanical and documented in BR-15.
...
| 1.4.0 | 2026-07-02 | Back Spec Agent | change | P2.1 canonical error-code taxonomy. BR-15 rewritten to publish the canonical namespaced code map (`VALIDATION_REQUIRED_FIELD` / `VALIDATION_INVALID_FORMAT` / `VALIDATION_OUT_OF_RANGE` / `RESOURCE_NOT_FOUND` / `SYSTEM_INTERNAL_ERROR`) — byte-identical on REST and on the MCP curation transport for `compliance_delete`; the pre-P2.1 §14 short-code set (`STRUCTURAL_INVALID` / `NOT_FOUND` / `INTERNAL`) is retired. BR-14 rewritten to make the byte-identical parity contract explicit (both handlers reuse the same `renderErrorEnvelope(failure)` helper; the `code/mcpCode` pair in `compliance-audit/service/errors.ts` is retired in the code-consequence PR of P2.1). §1 MCP server row, §1 Validation library row, §1 Testing row (parity assertions rewritten to check byte-identical namespaced codes on both transports; removed §14 short-code assertions), §6 External Integrations row (canonical envelope note), §7 Known Technical Constraints (new P2.1 bullet at the top; the "asymmetry" bullet reworded as "unified vocabulary" with pointer to `docs/specs/_global/error-codes.md`), and every "MCP `INTERNAL`" / "MCP envelope `INTERNAL`" / "envelope `STRUCTURAL_INVALID`" mention across BR-01 / BR-02 / BR-04 / BR-07 / BR-08 / BR-17 / BR-18 rewritten to name the canonical `SYSTEM_INTERNAL_ERROR` / `VALIDATION_*` codes. No change to: UCs, state machine ST-RI-DEL, BR-03 idempotency semantics, BR-06/BR-07 cascade predicates, BR-08 audit-row count, BR-18 `original_input` redaction rule, `content_hash` behaviour, schema, migrations, external integrations. No new BUSINESS_ codes for this domain; the mapping uses only existing global namespaced codes (`AUTH_UNAUTHORIZED`, `RESOURCE_NOT_FOUND`, `VALIDATION_REQUIRED_FIELD`, `VALIDATION_INVALID_FORMAT`, `VALIDATION_OUT_OF_RANGE`, `SYSTEM_INTERNAL_ERROR`). The seven §14 short codes are moved to the "Deprecated Codes" section of `docs/specs/_global/error-codes.md`; the new namespaced code `BUSINESS_LINK_RULE_VIOLATION` registered by that catalog under Ingestion is a placeholder that unblocks the paired `ingestion.back.md` reconciliation (out of this domain's scope). Companion updates: `curation.back.md` BR-30 (drop last-paragraph `compliance_delete` carve-out), `ingestion.back.md` BR-13/14/15 + Validation library (namespaced codes on both transports) — both scheduled in separate spec-writer runs. Code-consequence PR: centralise the REST↔MCP mapping in `backend/src/shared/error-mapping.ts`; retire the `code/mcpCode` pair in `compliance-audit/service/errors.ts` and its ingestion counterpart. No migration / DB change. | P2.1 |
