...
| Motion explains state change | Animation is semantic, not decorative: uncertain pulses, entity merges collapse two nodes into one, promotion (uncertain → accepted) eases in, supersession fades to grey. All wrapped in `prefers-reduced-motion`. |
...
|---|---|---|
| [`tokens.md`](./tokens.md) | Colors (incl. confidence states, NodeType colors, LinkType colors + temporal/stable stroke), spacing, typography, radius, shadow, glass-surface (3 levels), ambient-backdrop treatment, graph-depth overlay, 6 motion variants (4 state-change + 2 glass surface enter/exit), z-index scale | Whenever implementing visual styles |
| [`../components/StateBadge.component.spec.md`](../components/StateBadge.component.spec.md) | Confidence-state badge atom (one variant per state) | Implementing or invoking the state badge |
| [`../components/GlassSurface.component.spec.md`](../components/GlassSurface.component.spec.md) | Frosted-glass container atom (foundation of panels, drawer, popovers, modals, frame) | Building any floating layer or theme-aware glass region |

> Future waves will add `composition.md` (glass effects, hierarchy, density), `components.md` (catalog of design-system primitives vs. feature-local components), and `implementation.md` (a11y QA checklist, animations, team guidelines). **Out of scope in this foundation wave.**
