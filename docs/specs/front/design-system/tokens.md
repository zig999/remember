...
| `motion.graph.nodeReveal` | Graph nodes entering canvas (graph-improvement wave) | `opacity: 0 → 1` AND `scale: 0.85 → 1` | `--duration-fast` (200 ms) `--ease-out` | once (on node mount during graph population) |
