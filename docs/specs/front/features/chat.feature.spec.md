...
> **v1.4.0 note — `mapWireToGraphDelta` extraction:** the wire→surface mapping function `mapWireToGraphDelta` was formerly in `features/chat/api/_transforms.ts`. In the ingest wave it was extracted to `features/graph/api/mapWireToGraphDelta.ts` so that `features/ingest/api/useIngestGraphAssembly.ts` can import it without a cross-feature dependency. `features/chat/api/useSendMessage.ts` now imports from `features/graph/api/` (same name, new path). No behavior change; the type signatures and test coverage are unchanged.
