...
**Does NOT:**
- Own any graph state — pure presentational component (receives all data from React Flow's `EdgeProps`).
...
| `sourceX` / `sourceY` | `number` | yes (RF injected) | — | SVG coordinates of the source handle |
| `targetX` / `targetY` | `number` | yes (RF injected) | — | SVG coordinates of the target handle |
